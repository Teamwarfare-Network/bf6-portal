...
// version: 0.217 | Date: 01.01.26 | Time: 05:40 UTC
...
/*
 * ------ Need to enable/disable spawners per config (e.g. just 1 tank for 1v1, add a tank for 2v2, etc)
 * ------ Need complete list of spawn points per map, with vehicle list per map
 * ------ Need to implement map detection logic from deluca's tool 
 * ------ Code review, add regions for new features, review code structure
 * 
 ...
 * - Major Feature: Dynamic Vehicle selection dialog (choose tanks per player) so players decide vehicle they want?
 */
...
const ENABLE_DEBUG_NOTIFICATION_MESSAGES = true; // Green box notifications. Defaults to false.
const ENABLE_DEBUG_HIGHLIGHTED_MESSAGES = true; // White highlighted world log. Defaults to false.
...
type VehicleSpawnSpec = { pos: mod.Vector; rot: mod.Vector; vehicle: mod.VehicleList };
...
const ACTIVE_MAP_KEY: MapKey = "Blackwell_Fields";
...
const MAP_CONFIGS: Record<MapKey, MapConfig> = {
    ...
    Blackwell_Fields: {             
                                   //posX      posY      posZ 
        team1Base: mod.CreateVector(-151,      86,      -567), //West
        team2Base: mod.CreateVector( 388,      71,       136), //East
        vehicleSpawnYawOffsetDeg: 0, //-25? Needs testing, real T1 rot is 60, T2 rot is -115

        team1Spawns: [             //posX      posY      posZ                             posX      posY      posZ
            { pos: mod.CreateVector(-163.285,  86.96,   -570.254), rot: mod.CreateVector( 0.0,      45.0,     0.0),       vehicle: mod.VehicleList.Abrams       },
            { pos: mod.CreateVector(-169.559,  86.915,  -560.168), rot: mod.CreateVector( 0.0,      45.0,     0.0),       vehicle: mod.VehicleList.M2Bradley    },
            { pos: mod.CreateVector(-178.174,  87.159,  -551.206), rot: mod.CreateVector( 0.0,      45.0,     0.0),       vehicle: mod.VehicleList.Abrams       },
            { pos: mod.CreateVector(-185.833,  87.159,  -540.949), rot: mod.CreateVector( 0.0,      45.0,     0.0),       vehicle: mod.VehicleList.M2Bradley    },
        ],
        team2Spawns: [             //posX      posY      posZ                             posX      posY      posZ
            { pos: mod.CreateVector( 403.575,  71.227,   144.594), rot: mod.CreateVector( 0.0,     -90.0,     0.0),       vehicle: mod.VehicleList.Leopard      },
            { pos: mod.CreateVector( 404.054,  71.227,   131.966), rot: mod.CreateVector( 0.0,     -90.0,     0.0),       vehicle: mod.VehicleList.CV90         },
            { pos: mod.CreateVector( 403.575,  71.227,   118.571), rot: mod.CreateVector( 0.0,     -90.0,     0.0),       vehicle: mod.VehicleList.Leopard      },
            { pos: mod.CreateVector( 403.274,  71.227,   105.886), rot: mod.CreateVector( 0.0,     -90.0,     0.0),       vehicle: mod.VehicleList.CV90         },
        ],
    },
...
const ACTIVE_MAP_CONFIG = MAP_CONFIGS[ACTIVE_MAP_KEY]; // Expected to include team bases + at least one spawn per team.

// Baseline team inference from static main-base anchor coordinates (Option A).
// - Replace these vectors with exact map coordinates later.
// - A spawned vehicle is assigned to the nearest base anchor if within MAIN_BASE_BIND_RADIUS_METERS.
const MAIN_BASE_TEAM1_POS = ACTIVE_MAP_CONFIG.team1Base;
const MAIN_BASE_TEAM2_POS = ACTIVE_MAP_CONFIG.team2Base;
...
const TEAM1_VEHICLE_SPAWN_SPECS = ACTIVE_MAP_CONFIG.team1Spawns;
const TEAM2_VEHICLE_SPAWN_SPECS = ACTIVE_MAP_CONFIG.team2Spawns;
const VEHICLE_SPAWN_YAW_OFFSET_DEG = ACTIVE_MAP_CONFIG.vehicleSpawnYawOffsetDeg;

// Vehicle spawner tuning constants.
const VEHICLE_SPAWNER_START_DELAY_SECONDS = 1;
const VEHICLE_SPAWNER_RESPAWN_DELAY_SECONDS = 15;
const VEHICLE_SPAWNER_TIME_UNTIL_ABANDON_SECONDS = 5;
...
const VEHICLE_SPAWNER_BIND_DISTANCE_METERS = 150.0;
const VEHICLE_CAMPED_DISTANCE_METERS = 150.0;
...
const DEFAULT_AUTO_START_MIN_ACTIVE_PLAYERS = 1;
...
function addVehicleSpawnerSlot(teamId: TeamID, spawnPos: mod.Vector, spawnRot: mod.Vector, vehicleType: mod.VehicleList): number {
...
async function startVehicleSpawnerSystem(): Promise<void> {
    ...
    for (const spec of TEAM1_VEHICLE_SPAWN_SPECS) {
        addVehicleSpawnerSlot(TeamID.Team1, spec.pos, spec.rot, spec.vehicle);
    }
    for (const spec of TEAM2_VEHICLE_SPAWN_SPECS) {
        addVehicleSpawnerSlot(TeamID.Team2, spec.pos, spec.rot, spec.vehicle);
    }
...
export async function OnVehicleSpawned(eventVehicle: mod.Vehicle): Promise<void> {
    ...
    // Spawn-time registration is the authoritative path for scoring (before any player enters).
    ...
    const rot = mod.GetObjectRotation(eventVehicle);
    const rotY = Math.round(mod.YComponentOf(rot) * 100) / 100;
    sendHighlightedWorldLogMessage(
        mod.Message(mod.stringkeys.twl.messages.vehicleSpawned, teamNameKey, x, rotY),
        true
    );
}
...
// EOF version: 0.217 | Date: 01.01.26 | Time: 05:40 UTC