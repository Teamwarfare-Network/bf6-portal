...
// version: 0.322 | Date: 01.06.26 | Time: 08:13 UTC
 ...
 * 
 * Possible Bugs to identify/repo:
 * - 3rd tank is swapped/wrong? Defense Nexus, Blackwell Oil fields, - Leopard and Abrams swapped
 * - Attribution went to old person who owned tank previously (maybe before side swapping? - BUT the point did attribute correctly)
 * - Ammo restocked in the middle of the map? How?? Area 22B
 * - Add a dialog on top of Spawn screen to explain mode better? first time spawn?
 */
...
const SHOW_DEBUG_TIMELIMIT = false; // Show the top-right time limit debug text in the Ready dialog.
...
const ADMIN_PANEL_BG_ALPHA = 0.95;
...
const STR_JOIN_PROMPT_BODY = mod.stringkeys.twl.joinPrompt.body;
...
function ensureHudForPlayer(player: mod.Player): HudRefs | undefined {
    ...
    // --- Counter widgets (digits) ---
    {
        ...
        refs.roundCurText = mkCounter(`RoundCounterContainer_${pid}`, `RoundCounterText_${pid}`, [970.93, 44.35], [13.95, 18.15], 16);
        refs.roundMaxText = mkCounter(`RoundCounterMaxContainer_${pid}`, `RoundCounterMaxText_${pid}`, [993.03, 44.35], [13.95, 18.15], 16);
                    ...
                    children: [
                        {
                            ...
                            bgAlpha: 0,
                            bgFill: mod.UIBgFill.None,
                            textLabel: mod.Message(mod.stringkeys.twl.hud.roundStateFormat, 0, mod.stringkeys.twl.hud.labels.roundKills, getTeamNameKey(TeamID.Team1)),
                        ...
                        },
                        {
                            ...
                            bgAlpha: 0,
                            bgFill: mod.UIBgFill.None,
                            textLabel: mod.Message(mod.stringkeys.twl.hud.roundStateFormat, 0, mod.stringkeys.twl.hud.labels.roundKills, getTeamNameKey(TeamID.Team2)),
...
async function scheduleRoundEndCleanup(expectedToken: number): Promise<void> {
    ...
    // Force-deploy pass: AutoSpawn + global deploy, then per-player fallback for stragglers.
    ...
    const players = mod.AllPlayers();
    const pCount = mod.CountOf(players);
...
async function scheduleFinalRoundVictory(expectedToken: number, winningTeamNum: TeamID | 0): Promise<void> {
    ...
    // Final-round exception: re-enable deploy so Victory dialog is visible.
...
//#region -------------------- Join Help Prompt (spawn screen overlay) --------------------
...
function createJoinPromptForPlayer(player: mod.Player): void {
    ...
    mod.AddUIContainer(
        ...
        mod.CreateVector(0, -36, 0),
        mod.CreateVector(900, 480, 0),
        mod.UIAnchor.Center,
        root ?? uiRoot,
        true,
        0,
    ...
    mod.AddUIText(
        ...
        0,
        mod.UIBgFill.None,
        mod.Message(STR_JOIN_PROMPT_BODY),
        22,
        mod.CreateVector(1, 1, 1),
        1,
    ...
    if (neverShowButton) {
        ...
        mod.EnableUIButtonEvent(neverShowButton, mod.UIButtonEvent.ButtonUp, true);
    }
    const neverShowBorderWidget = safeFind(joinPromptNeverShowButtonBorderName(pid));
    if (neverShowBorderWidget) mod.SetUIWidgetDepth(neverShowBorderWidget, mod.UIDepth.AboveGameUI);
    ...
    if (neverShowText) {
        ...
        mod.SetUIWidgetPosition(neverShowText, neverShowBorder ? mod.CreateVector(0, 0, 0) : mod.CreateVector(-200, 173, 0));
    }
    ...
    const dismissText = addCenteredButtonText(
        ...
        360,
        70,
        mod.Message(STR_JOIN_PROMPT_DISMISS),
        player,
        dismissTextParent
    );
    if (dismissText) {
        ...
        mod.SetUIWidgetPosition(dismissText, dismissBorder ? mod.CreateVector(0, 0, 0) : mod.CreateVector(200, 173, 0));
    }
}
...
//#endregion ----------------- Join Help Prompt (spawn screen overlay) --------------------
...
export async function OnPlayerDeployed(eventPlayer: mod.Player) {
    ...
    State.players.deployedByPid[getObjId(eventPlayer)] = true;
...
// EOF version: 0.322 | Date: 01.06.26 | Time: 08:13 UTC