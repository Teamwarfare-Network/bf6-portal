...
export async function OnVehicleDestroyed(eventVehicle: mod.Vehicle) {
    ...
    if (isRoundLive()) {
        ...
        if (!wasSpawnCamped) {
            ...
            if (
                State.scores.t1RoundKills >= State.round.killsTarget ||
                State.scores.t2RoundKills >= State.round.killsTarget
            ) {
        ...
        if (!wasSpawnCamped) {
            ...
            const teamNameKey =
                scoringTeamNum === TeamID.Team1 ? getTeamNameKey(TeamID.Team1) : getTeamNameKey(TeamID.Team2);
            const ownerNameOrKey = owner
                ? mod.IsPlayerValid(owner)
                    ? owner
    ...
    const destroyedTeamNameKey =
        destroyedTeamNum === TeamID.Team1 ? getTeamNameKey(TeamID.Team1) : getTeamNameKey(TeamID.Team2);
    const ownerNameOrKey = owner
        ? mod.IsPlayerValid(owner)
            ? owner
            : mod.stringkeys.twl.system.unknownPlayer
