...
### Wave 8 — Spectator camera (SHIPPED v1.475–v1.486, 2026-05-08)
A new gameplay surface, not a reclaim wave. Adds a **single-spectator-slot Mitigation C integration** so a coach / observer can watch a match without occupying a player slot, without holding a vehicle, and without firing weapons. Built on a Godot-authored `FixedCamera` (Operation Firestorm only at ship; other 8 maps deferred behind graceful-degradation that keeps the SPECTATE button disabled-grey). Plan archives: [`5.08.26_conquest_spectator_cam_plan.md`](./5.08.26_conquest_spectator_cam_plan.md), [`5.08.26_conquest_spectator_cam_ship2_plan.md`](./5.08.26_conquest_spectator_cam_ship2_plan.md), [`5.08.26_conquest_spectator_cam_ship3_plan.md`](./5.08.26_conquest_spectator_cam_ship3_plan.md), [`5.08.26_conquest_spectator_cam_ship4_plan.md`](./5.08.26_conquest_spectator_cam_ship4_plan.md).

- **Ship 1 (v1.475–v1.477) — Probe.** Empirically resolved gates I-4 (`SetCameraTypeForPlayer` runtime acceptance on undeployed players: **FAIL** — forces Mitigation C) and I-12 (engine spectator UI overlay competing with our Fixed camera: **PASS**). Probe file deleted in Ship 2.
- **Ship 2 (v1.478) + follow-up (v1.479) — Single-slot integration.** New `State.players.spectatorPid: number | null` (single nullable scalar — NOT pid-indexed; one spectator at a time is the canonical model). New files [`spectator-action.ts`](../src/spectator/spectator-action.ts) + [`coach-button-sync.ts`](../src/spectator/coach-button-sync.ts). COACH wired on both surfaces (panel + admin's full ready dialog). Click attaches `(Cameras as any).Fixed` directly (no DeployPlayer round-trip; the player was already deployed when they clicked because triple-tap is gated on deployed). Combat HUD persists per user direction so the spectator watches tickets/flags/captures. Boundary skip + defensive `OnPlayerDeployed` re-attach hook. v1.479 added: 18-input lock (movement / combat / interact / weapon-slot — body still walked at HQ in v1.478 because camera swap doesn't touch input); PCT-pattern sealed underground hide-room (4 walls + floor + ceiling at `camPos + (0, -50, 0)`); `mod.DisplayNotificationMessage` self-confirm.
- **Ship 3 (v1.481+v1.482) + SP fixes (v1.483) — Free Camera (PCT pattern).** Per-tick `mod.SetObjectTransform` loop runs at ~30 Hz while spectator slot is held. Reads `LinearVelocity` decomposed against `FacingDirection` for horizontal motion, `IsJumping`/`IsCrouching` for ±Y vertical, `IsSprinting` for ×2.5 multiplier (later ×3.5). Pitch clamped to ±89°, smoothing factors 0.12 (forward/strafe) / 0.18 (rotation/vertical) lifted from PCT empirical tuning. Module-local `_spectatorFreeCameraLoopToken: number` provides one-tick cancellation (cleanup hooks bump the token). Reduced lock list to 13 inputs (released movement / sprint / jump / crouch). Ship 3 SP feedback (v1.483): hide-room moved from camera-relative `camPos.y - 50` to absolute `Y = -500` (PCT's relative offset assumed ground-level cinematic cameras; Firestorm's spec cam at Y≈246 meant the room sat at Y≈196, blocking aircraft). Sky-fallback + water-check logic dropped (water can't reach Y=-500). Pitch inverted via `-Math.asin(...)` for engine convention.
- **v1.484/v1.485 — speed tuning.** SP feel-test loop. Walk gain 1.0 → 3.0 → 10.0; sprint multiplier 2.5 → 3.5 (sprint speed = 35). Vertical kept at 20.0 base.
- **Ship 4 (v1.486) — Exit Mechanism.** Triple-tap interact returns spectator to deploy screen on foot. New `exitSpectatorMode(player, pid)` (~35 LOC) mirrors `enterSpectatorMode` in reverse. Routing in `ongoingPlayerImpl` ([player-loop-inputs.ts:3-18](../src/index/player-loop-inputs.ts#L3-L18)) early-returns on the spectator pid path. Required prep: dropped `RestrictedInputs.Interact` from the lock list (12 inputs now, was 13) so `IsInteracting` SoldierStateBool flips cleanly; safe because the body is in a 5mm sealed cube at Y=-500 with no InteractPoint authoring in range. Button label "SPECTATE / COACH" → "SPECTATE". COACH-button lockout was already correct in `coach-button-sync.ts:30-37` — no change there; the post-exit refresh fan-out re-enables the button automatically.

**Net cost across the arc:** +13,800 bundle bytes; **zero new M-tier per-pid storage** (the only state added is a single nullable scalar). Out-of-scope follow-ups (deferred from Ship 3/4): player-target tracking on Jump key (Ship 3b, ~50 LOC), raycast wall correction (Ship 3c, ~80 LOC), multi-spectator support, spectator team-scoreboard hide, mid-match LIVE entry, Reload-key shortcut for exit (deferred until empirical SDK test confirms `IsReloading` flips with Reload restricted).
