...
| 94 | CQ_Bug_GetInventoryAmmo_SupplyBox_OpenMenu | **Open — re-observed v1.408** | v1.408 | Cosmetic engine log re-emerged in v1.408 SP smoke test (Medic class, opening Supply Box, selecting medic gadgets / smoke arty). Class-agnostic (not Assault-specific as v1.372B hypothesis suggested). NOT a Wave 1/2 regression — pre-existing latent bug, v1.379 "not reproducing" mark was premature. Three fix paths documented in the issue body; not actioned yet. |
...
| 109 | CQ_Bug_16Player_Playtest_JS_Memory_Limit | **Open — Critical (playtest-blocking)** | v1.406 | Mod Evaluator terminated the script at load with `Mod has reached its js script memory usage limit. It has been terminated` during a 16-player MP playtest. Bundle is 872,014 bytes (well below the 1 MiB upload cap), so failure axis is runtime JS heap, not upload size. Suspected drivers: per-player widget caches in `State.hudCache.*` (~hundreds of widget refs × N players), per-player view-model snapshots, ~30 PID-keyed records, 3,402 module-level identifiers (373 from UI layout alone), ~8.8KB dead strings.json keys, and closures held by `Timers.setTimeout` continuations. Documentation-only first pass; reclaim plan tracked in `conquest_optimization_analysis.md`. |
...
Status: **Open — re-observed v1.408 (2026-04-27)**. Originally captured in v1.372B; marked "Resolved (not reproducing)" at v1.379 prematurely. Re-emerged in v1.408 SP smoke test ([screenshot](../reference_design_documentation/testing_images/20260427212633_1.jpg)) — `GetInventoryAmmo` + `GetInventoryMagazineAmmo` engine errors fired multiple times when a Medic player opened the Supply Box and selected medic gadgets / smoke artillery. **Class-agnostic** (rules out the v1.372B Assault-specific hypothesis). **Not a Wave 1/2 regression** — pre-existing latent bug; Waves 1 (v1.407) and 2 (v1.408) did not modify `ammo-resupply-menu.ts`. Cosmetic only — gameplay continues, no JS exceptions, just noisy world-log overlay. Three fix paths documented in the issue body: (1) min-effort try/catch wrap at the four unwrapped sites, (2) preferred precheck-via-`HasEquipment` to silence the engine log, (3) deferred architectural refactor to use `State.players.lockerSlots[pid]` as authoritative slot source. Not actioned in current optimization arc; user wants focus on Wave 3+.
...
Status: **Open — Critical (playtest-blocking) (v1.406, 2026-04-27).** Mod Evaluator terminated the script at load during a 16-player MP playtest with: `ERROR REPORTED BY MOD EVALUATOR WHILE RUNNING JS SCRIPT — Mod has reached its js script memory usage limit. It has been terminated`. Functionality is confirmed-good in low-player-count testing; failure mode is purely runtime heap capacity. Bundle size is 872,014 bytes — **well below** the 1,048,576-byte upload cap (~17% headroom) — so the upload byte cap is **not** the failure axis. Per-player heap multipliers (16 players × per-pid widget caches + view-model snapshots + ~30 PID-keyed records) are the most plausible vector. Suspected contributors ranked by static-analysis weight: (1) `State.hudCache.*` widget caches — `vehicleDeployTimerCache`, `ammoResupplyMenuCache`, `topHudShellByPid`, `boundaryPromptCache`, `clockWidgetCache`, `countdownWidgetCache`, hundreds of `mod.UIWidget` refs per pid; (2) per-player view-model objects in `State.conquest.debug.*` (`hudStatusVmByPid`, `hudHelpReadyVmByPid`, `hudClockVmByPid`); (3) Combat HUD entry graph from `twlConquestHudEnsurePlayerGraph` — tickets/flags/engage/clock with stacked shadow-ring text widgets per glyph; (4) `AmmoResupplyMenuCacheEntry.rows` arrays-of-objects-of-widget-refs sized by gadget config; (5) ~30 `Record<number, T>` per-pid maps initialized lazily; (6) 3,231 module-level `const` + 171 `let` (3,402 retained module-level identifiers) — UI-layout constants alone account for 373; (7) ~8.8KB of dead `joinPrompt.*` and feature-flagged strings still bundled in `bundle.strings.json` (FEATURE_* flags do not gate strings); (8) closures captured by `Timers.setTimeout`/`mod.Wait` continuations holding pid + player + incidental locals. No engine telemetry exposed to confirm — Portal SDK does not surface a `mod.GetScriptMemory*` or comparable API (verified against `reference_sdk_1.2.3/code/types/mod/index.d.ts`). Reclaim plan tracked in [`conquest_optimization_analysis.md`](./conquest_optimization_analysis.md). Documentation-only first pass; no code changes in this branch. Verification gate: re-run 16-player playtest after reclaim ships and confirm full match start-to-victory without termination.
