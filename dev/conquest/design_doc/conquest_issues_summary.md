...
Last updated: 2026-05-03 (v1.448) — closed #34/#78/#85/#88/#89 per user direction; #34 + #85 reclassified by-design / accepted, #78 + #88 + #89 confirmed resolved. Player-facing impact reclassification: #3 / #16 / #86 dropped from the player-known-issues list as low-impact (they remain Open internally). #100 narrowed: only fires in B+C-owned-without-A state; future fix lands with the custom player spawner system. New: #113 added — console controller cannot activate Vehicle Deploy SPAWN buttons from the deploy screen; on-foot purple-smoke live terminal works as a workaround.
...
| 113 | CQ_Bug_Console_DeployScreen_Vehicle_Buttons_Unresponsive | Open | v1.448 | Console / controller players cannot activate the SPAWN buttons in the Vehicle Deploy menu from the deploy screen. Same menu opened on-foot via the purple smoke at HQ works correctly. Suspected input-mode / cursor-routing gap on the deploy-screen reveal path. Workaround: deploy on foot, then use the purple smoke live terminal. |
...
### #113 — CQ_Bug_Console_DeployScreen_Vehicle_Buttons_Unresponsive
Status: Open (2026-05-03, captured for player-facing known-issues list). On controller / console, the SPAWN buttons in the Vehicle Deploy menu surfaced on the deploy screen do not respond to controller input — the menu renders correctly (rows visible, statuses correct, WAIT / READY labels accurate) but pressing A / X on a row produces no spawn request. Same menu opened on-foot via the purple smoke triple-tap at HQ works correctly on console. PC mouse input works in both contexts. Suspected: input-mode / cursor-routing discrepancy between the two open paths — the live-terminal path explicitly calls `setUIInputModeForPlayer(eventPlayer, true)` in `tryOpenVehicleDeployLiveMenuForPlayer`; the deploy-screen reveal path does not. Workaround: deploy on foot, then use the purple smoke. Surfaced on player-facing known issues; awaits controller-input investigation.
