...
## TL;DR (v1.474 measurements; v1.406 baseline noted inline for delta)

> **Re-measured v1.474, 2026-05-08.** Numbers below reflect the current state per direct measurement of `dist/bundle.ts`. v1.406 baseline values are bracketed for delta comparison. The qualitative conclusions are unchanged — bundle is still well under the 1 MiB cap, runtime heap remains the binding constraint at 16+ players.

1. **Failure mode shifted.** The 1 MiB upload cap is no longer the binding constraint. Bundle is **865,714 bytes** at v1.474 *(was 872,014 at v1.406; net −6,300 across the Wave 1-6 + bug-fix + post-v1.454 arc — most of which came from v1.464's vehicle-deploy chrome reclaim, partially given back by the v1.471 late-join crash defense's +420 bytes and v1.473's +294 bytes for new Firestorm supply boxes)*, with **17.44% headroom** under the cap. The new constraint is the **Mod Evaluator runtime JS memory ceiling**, hit at 16 concurrent players. Memory-pressure work supersedes the prior bundle-byte-pressure work.
