...
## TL;DR (v1.454 measurements; v1.406 baseline noted inline for delta)

> **Re-measured v1.454, 2026-05-03.** Numbers below reflect the current state per direct measurement of `dist/bundle.ts`. v1.406 baseline values are bracketed for delta comparison. The qualitative conclusions are unchanged — bundle is still well under the 1 MiB cap, runtime heap remains the binding constraint at 16+ players.

1. **Failure mode shifted.** The 1 MiB upload cap is no longer the binding constraint. Bundle is **869,724 bytes** at v1.454 *(was 872,014 at v1.406; net −2,290 across the Wave 1-6 + bug-fix arc)*, with **17.06% headroom** under the cap. The new constraint is the **Mod Evaluator runtime JS memory ceiling**, hit at 16 concurrent players. Memory-pressure work supersedes the prior bundle-byte-pressure work.
...
|:-------:|:-----:|-----------|--------|-----------------------|---------------------|-------|
| **M1** | **XL** | `State.hudCache.vehicleDeployTimerCache[pid]` | [state/hud-cache-types.ts:104](../src/state/hud-cache-types.ts#L104) | **Re-measured v1.454: ~310 widget refs/pid** — 10 root widgets (root, livePanelBlur×3, closeButton×3, closeButtonText/Shadow) + 12 rows × 25 widgets each (18 main row widgets + 7 in nested `ReusableTimerWidgetCacheEntry` post-Wave-5 + v1.446 `barText`). Doc previously assumed "~6-10 typical rows" but `VEHICLE_DEPLOY_TIMER_MAX_ROWS = 12` (foundation/ui-layout.ts) — slot count is a fixed cap, not variable. | **~4,960 widget refs** at 16p *(v1.406 doc claimed 2,400-4,000; re-measure shows previous estimate was ~24% low due to underestimating row count)* | `VehicleDeployTimerRowCacheEntry` per row carries `lastShowSpawnButton`, `lastShowGroundButton`, `lastSpawnButtonVisualState`, etc. — diff-cache fields that are flag-state mirrors. v1.446 added `barText` widget on every reusable timer instance via `buildReusableTimerBarText` (+1 widget/row × 12 rows × 16p = +192 refs). v1.454 doc audit corrected the row count assumption — M1 is **the largest single per-pid heap allocator** at ~4,960 refs at full lobby; reclaim ROI for M1 is even higher than the doc previously suggested. |
...
> **Re-measured v1.454, 2026-05-03.** O1/O2 counts directly grep'd from `dist/bundle.ts` (`^const ` / `^let ` patterns). O3 byte count from `wc -c dist/bundle.strings.json`. Methodology note for O1: the v1.406 measurement (3,231) used an unspecified tool; the v1.454 grep counts only `^const ` line starts and may differ slightly from a full AST scan. Both numbers represent module-level declarations.

| # | Source | Quantity (v1.454) | v1.406 baseline | Delta | Reclaim feasibility |
|---|--------|-------------------|-----------------|-------|---------------------|
| O1 | Module-level `const` declarations in bundle | **3,319** | 3,231 | +88 (+2.7%) | High — many are layout constants whose value is set once and never reassigned. They could be *inlined* at use sites, removing the binding from the heap. |
...
| O3 | `bundle.strings.json` keys | **22,578 bytes** | 22,082 | +496 (+2.2%) | Medium — ~8.8 KB are dead (Cat 8 below). Strings live separately from script heap but still occupy runtime memory. |
