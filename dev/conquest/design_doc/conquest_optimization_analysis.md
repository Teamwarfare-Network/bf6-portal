...
## TL;DR (v1.494 measurements; v1.406 baseline noted inline for delta)

> **Re-measured v1.494, 2026-05-09.** Numbers below reflect the current state per direct measurement of `dist/bundle.ts`. v1.406 baseline values are bracketed for delta comparison. The qualitative conclusions are unchanged — bundle is still well under the 1 MiB cap, but the binding constraint is now **per-frame CPU cap** (Tier S, post-v1.491) rather than heap (Tiers A-F, post-v1.406).

1. **Failure mode shifted twice.** (a) v1.406: 16-player heap-OOM drove waves 1–6 of heap reclaim (Tiers A–F). (b) v1.491: 8–10 player per-frame CPU spike (1,716ms / 1,000ms cap) drove the Tier S framework. Bundle is **925,049 bytes** at v1.494 *(was 872,014 at v1.406; +53,035 net across the entire arc — Waves 1-6 reclaimed ~6,300 bytes which was outweighed by v1.492's +42,839 bytes when `FEATURE_ADMIN_PANEL` + `FEATURE_POSITION_DEBUG` were re-enabled to support the admin diagnostic toggles)*, with **11.78% headroom** under the cap. The 1 MiB upload cap is no longer the binding constraint. The current binding constraint is the **per-frame eval budget** (1,000ms hard cap, opaque). Heap remains a secondary concern but appears stable post-Wave-6. Tier S work is the active ship target; Tier A work resumes after MP stability is restored.
...
|---|------|-----------|---------|-----------|:--------:|--------------------------------------------------------------------------|
| **S1** | `updateConquestCombatHudForAllPlayers(force=true)` from `OnPlayerEnterCapturePoint` | per enter event | O(N players × M VM-build) | **NONE — `force=true` bypasses 200ms gate** | 🔴 | **Coalesce.** 50ms `Timers.setTimeout` debouncer collapses cluster of N enters into 1 broadcast. Pattern: Wave 6 Ship 2 coalesced post-leave refresh. **Risk:** flag visual-flip immediacy may need to stay forced — needs SP test of "does flag color flip feel synchronous if delayed 50ms?" |
| **S2** | `updateConquestCombatHudForAllPlayers(force=true)` from `OnPlayerExitCapturePoint` | per exit event | O(N players × M VM-build) | **NONE — bypasses gate** | 🔴 | Same coalescing as S1; share the debounce timer with enters since they bracket the same flag-cluster. |
| **S3** | `updateVehicleDeployTimerHudForAllPlayers()` from `OnPlayerEnter/ExitVehicle` | per vehicle event | O(N × 16 row sigs) | None | 🔴 | **Throttle to dirty-flag + 200ms.** Same shape as combat HUD's existing throttle. The vehicle-deploy timer's render plan signature already exists (`lastRenderSignature` diff cache); the broadcast is wasteful when the signature doesn't change. |
...
| **S10** | Boundary classifier safety-net re-probe (`safeGetSoldierStateBool(IsInVehicle)` for on-foot above ceiling) | per OOB classify | O(misclassify rate × N) | None | 🟡 | The user reports chopper occupants getting flagged OOB at altitude → safety-net keeps re-probing. Independent **correctness bug** (vehicle attribution lost for chopper passengers); fixing the upstream classification fixes both the spike compound AND the false-OOB-kill UX. |
...
### Open questions for user (Tier S)

1. **S1/S2 — is "atomic enter/exit visual flip" load-bearing?** The comment at [`area-triggers.ts:46,65`](../src/index/area-triggers.ts#L46) says "Enter/exit should feel atomic: apply top row + popout + engage in one immediate pass." That justifies `force=true`. **Question:** is a 50ms coalesced delay perceptible to players? If not, the throttle becomes acceptable. If yes, an alternative is needed (e.g. force only the engaging-pid's HUD, broadcast-non-force to others — single forced pass per event becomes single-pid).
2. **S3 — vehicle-event broadcast throttling.** Is there any case where a vehicle deploy timer must update on the same frame as an enter/exit? (Probably not — the timer surface is latent; the engaging player's deploy menu, not the vehicle they entered.)
3. **S10 — vehicle attribution.** Does the engine `IsInVehicle` SoldierStateBool fire reliably for helicopter passengers? If not, the boundary safety-net is treating a state-bool gap as on-foot, which is wrong. Need an empirical SP test of `safeGetSoldierStateBool(IsInVehicle)` while seated as passenger in an AH-6M / Black Hawk / Apache.
