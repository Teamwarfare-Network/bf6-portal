...
## TL;DR (v1.468 measurements; v1.406 baseline noted inline for delta)

> **Re-measured v1.468, 2026-05-05.** Numbers below reflect the current state per direct measurement of `dist/bundle.ts`. v1.406 baseline values are bracketed for delta comparison. The qualitative conclusions are unchanged — bundle is still well under the 1 MiB cap, runtime heap remains the binding constraint at 16+ players.

1. **Failure mode shifted.** The 1 MiB upload cap is no longer the binding constraint. Bundle is **867,288 bytes** at v1.468 *(was 872,014 at v1.406; net −4,726 across the Wave 1-6 + bug-fix + post-v1.454 arc — most of which came from v1.464's vehicle-deploy chrome reclaim)*, with **17.30% headroom** under the cap. The new constraint is the **Mod Evaluator runtime JS memory ceiling**, hit at 16 concurrent players. Memory-pressure work supersedes the prior bundle-byte-pressure work.
