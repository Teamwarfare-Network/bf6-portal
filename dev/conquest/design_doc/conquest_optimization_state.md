...
## Project Stats (v1.474)

**Bundle:** 865,714 / 1,048,576 bytes (17.44% headroom). Net delta v1.470 → v1.474: +657 bytes across 3 shipped versions (v1.471 +420, v1.473 +294, v1.474 −57; v1.472 was skipped by the bumpVersion script). No PPM table changes since v1.470 — all three recent ships are surgical and preserve per-pid widget topology. The bundle remains well under the upload cap; runtime JS heap is still the binding constraint at 16+ players (see [`conquest_optimization_analysis.md`](./conquest_optimization_analysis.md) regime-change section).
