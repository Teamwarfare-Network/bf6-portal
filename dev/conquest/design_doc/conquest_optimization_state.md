...
## Project Stats (v1.486)

**Bundle:** 884,067 / 1,048,576 bytes (15.69% headroom; +1,271 bytes from v1.485). Net delta v1.474 → v1.486: +13,800 bytes across the entire spectator integration arc (Ship 1 probe v1.475–v1.477: +2,088; Ship 2 v1.478: +2,466; Ship 2 follow-up v1.479: +7,730; Ship 3 v1.481+v1.482: +5,518; Ship 3 SP-feedback v1.483: −710; v1.484/v1.485 speed tuning: ~+30 bytes literal-byte deltas; Ship 4 exit v1.486: +1,271). No PPM table changes — `State.players.spectatorPid` is still a single nullable scalar; `_spectatorFreeCameraLoopToken` is module-local; the per-tick loop is single-spectator scope, no per-pid record needed. Spectator domain now sums to ~620 LOC across 2 files (`spectator-action.ts` 572 LOC, `coach-button-sync.ts` 77 LOC) plus 1 wired call-site in `index/player-loop-inputs.ts`.
...
| **admin/** | | | | | |
| `admin/identity.ts` | 64 | ~2,560 | Y | — | Single-slot admin model. Claim-only — no auto-promotion. Module-level state: `_currentAdminPid: number \| undefined`. Public API: `onPlayerLeave(pid)` (auto-vacates admin slot if leaver was admin + broadcasts via `refreshAllVisiblePlayerReadyPanels`), `isAdmin(pid)`, `getCurrentAdminPid()`, `isAdminVacant()`, `claimAdmin(pid)` (succeeds only if vacant), `giveUpAdmin(pid)` (succeeds only if pid is current admin). Wired into `onPlayerLeaveGameImpl` only. **v1.459 (Strip Host)**: deleted `_hostFirstPid`, `_hostNameMessage`, `isHost()`, `getHostFirstPid()`, `getHostNameMessage()`, and the `onPlayerJoin` first-ever-join auto-admin branch entirely. Host was never script-authoritative (engine reports closest-to-server player as first-loaded, not actual server host). Net file shrink: ~110 → 64 lines, ~5,400 → ~2,560 bytes. |
| **admin-panel/** | | | | | |
...
| **spectator/** | | | | | |
| `spectator/spectator-action.ts` | 572 | 30,954 | Y | — | Single-spectator-slot integration (Mitigation C; v1.475–v1.486). Owns `enterSpectatorMode`, `exitSpectatorMode`, `runSpectatorFreeCameraLoop` (~30 Hz per-tick `mod.SetObjectTransform` loop driven by `LinearVelocity`/`FacingDirection` decomposition + `IsJumping`/`IsCrouching`/`IsSprinting` reads), PCT-pattern hide-room (4 walls + floor + ceiling at absolute Y=-500), 12-input lock, `OnPlayerDeployed` re-attach hook, 3 lifecycle deallocators (disconnect / match-end / fresh-setup). No per-pid state; module-local `_spectatorHideRoomObjects[]`, `_spectatorHideRoomCenter`, `_spectatorFreeCameraLoopToken` are sufficient because only one spectator can claim the slot at a time. |
| `spectator/coach-button-sync.ts` | 77 | 4,000 | Y | — | Per-pid COACH button visual+enable state for both surfaces (Player Ready Panel + admin's full ready dialog). State matrix encoded in `isCoachButtonEnabledForPid`: enabled only when `(camera authored && !isMatchLive && slot vacant && pid !== slot)`. Visual treatment shared via `applyCoachButtonState`. Refresh path is the existing panel + dialog broadcast helpers — no separate refresh broadcast. **No change in v1.486** — the spectator's own button correctly disables on claim and re-enables on exit via the existing `slot === pid` guard. |
| **state/** | | | | | |
...
Module: single-slot admin model. Claim-only — no auto-promotion. `Admin` namespace with one module-private field + 6 public accessors.
- `Admin.onPlayerLeave()` (1 — `onPlayerLeaveGameImpl`) — vacates admin slot if leaver was admin and broadcasts via `refreshAllVisiblePlayerReadyPanels`.
- `Admin.isAdmin()` (2 — router + ui-events-ready guard) — returns true iff `_currentAdminPid === pid`
- `Admin.getCurrentAdminPid()` (1 — panel refresh) — returns the current admin pid or undefined
- `Admin.isAdminVacant()` (1 — panel claim-button visibility) — returns true iff `_currentAdminPid === undefined`
- `Admin.claimAdmin()` (1 — CLAIM ADMIN handler) — atomic: succeeds only if `isAdminVacant()`; returns true on success, false on race (someone else claimed first)
- `Admin.giveUpAdmin()` (1 — GIVE UP ADMIN handler) — atomic: succeeds only if `pid === _currentAdminPid`; returns true on success, false otherwise

**v1.459 (Strip Host)**: removed `Admin.onPlayerJoin`, `Admin.isHost`, `Admin.getHostFirstPid`, `Admin.getHostNameMessage` (4 functions); removed `_hostFirstPid` + `_hostNameMessage` module state. Host was never script-authoritative.
...
- `isSpectatorAvailableForActiveMap()` (1 — `coach-button-sync.ts`) — gates COACH enable state on `ACTIVE_MAP_CONFIG.spectatorCameraId !== undefined`
...
- `enterSpectatorMode()` (2 — `ui-events-player-ready-panel.ts` COACH branch + `ui-events-ready.ts` COACH branch) — top-level claim: state writes + camera attach + input lock + hide-room spawn + teleport + free-cam loop start + world-log + notification self-confirmation + cross-viewer broadcast + auto-start check
...
- `isCoachButtonEnabledForPid()` (2 — internal, called by both panel + dialog sync) — encodes the COACH state matrix (camera authored && !isMatchLive && slot vacant && not own pid)
- `applyCoachButtonState()` (2 — internal, shared visual treatment helper) — sets button enabled + bg color/alpha + border color/alpha + label color
