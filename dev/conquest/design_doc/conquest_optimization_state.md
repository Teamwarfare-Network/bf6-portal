...
| ~~**L2**~~ | ~~`State.players.lockerSlotToggle[pid]`~~ | [interaction/ammo-resupply-menu.ts:1254](../src/interaction/ammo-resupply-menu.ts#L1254) (write), [index/player-join-leave.ts:203](../src/index/player-join-leave.ts#L203) (paired delete v1.500) | First locker-class toggle (lazy on first use) | (was) one entry per pid that ever opened locker | N/A — paired | **FIXED v1.500 (A13).** `delete State.players.lockerSlotToggle[pid]` added to `onPlayerLeaveGameImpl`. Rejoiner gets fresh `[2,2,2,2]` defaults. |
| ~~**L3**~~ | ~~`adminPanelLastPrimaryClickByPid` (module-local, NOT in `State.players`)~~ | [admin-panel/events.ts:5](../src/admin-panel/events.ts#L5) (decl), [admin-panel/events.ts:9-11](../src/admin-panel/events.ts#L9) (paired helper v1.500) | First admin click | (was) one number/pid per admin-claim cycle | N/A — paired | **FIXED v1.500 (A12).** New `resetAdminPanelPrimaryClickTrackerForPid` helper called from `cleanupHudForPid`. |
| ~~**L4**~~ | ~~`readyDialogLastPrimaryClickByPid` (module-local)~~ | [interaction/ui-events-ready.ts:8](../src/interaction/ui-events-ready.ts#L8) (decl), [interaction/ui-events-ready.ts:12-14](../src/interaction/ui-events-ready.ts#L12) (paired helper v1.500) | First ready-dialog primary click | (was) ~16 entries within a match | N/A — paired | **FIXED v1.500 (A12).** New `resetReadyDialogPrimaryClickTrackerForPid` helper called from `cleanupHudForPid`. |
| ~~**L5**~~ | ~~`playerReadyPanelLastPrimaryClickByPid` (module-local)~~ | [interaction/ui-events-player-ready-panel.ts:30](../src/interaction/ui-events-player-ready-panel.ts#L30) (decl), [interaction/ui-events-player-ready-panel.ts:34-36](../src/interaction/ui-events-player-ready-panel.ts#L34) (paired helper v1.500) | First player-ready-panel primary click | (was) same as L4 | N/A — paired | **FIXED v1.500 (A12).** New `resetPlayerReadyPanelPrimaryClickTrackerForPid` helper called from `cleanupHudForPid`. |
...
**Audit punchlist closed v1.500.** All 5 confirmed script-side leaks are now plugged: L1 dead-code-deleted v1.496; L2 + L3-L5 plugged via A13 + A12 in v1.500 (initial v1.498 attempt broke via non-ASCII em-dash, reverted in v1.499, re-shipped clean in v1.500). The largest (L1) was ~240 entries × ~50 B = ~12 KB per 20-min match; the rest were ~80 B per pid each. None were individually large enough to explain a 1,716ms-frame CPU spike via heap pressure alone — and **the script restarts at match end, so the v1.491 spike is a within-match accumulation problem, not a multi-match drift problem.**

**Implication.** With the script-side audit punchlist closed AND v1.497 S3 throttling shipped, the v1.491 within-match driver is most likely NOT pure script-side heap accumulation OR the vehicle-event broadcast cadence. It is more likely:
...
- (b) **Another Tier S burst event** (S1/S2 combat HUD enter/exit; S4–S10 not yet ranked) whose trigger conditions become more likely as more in-match game state accumulates.

The MP diagnostic-toggle playtest is now the gating signal — the script side is as clean as static analysis can make it.

### Recommended sequencing (post-audit, all leaks now plugged)
...
2. ~~**Plug L3-L5 in one ship (A12)**~~ — **SHIPPED v1.500.** 3 exported `reset*PrimaryClickTrackerForPid` helpers (mirror of existing vehicle-deploy helper) called from `cleanupHudForPid`. Bundle delta: folded into v1.500 +563 B total.
3. ~~**L2 (A13)**~~ — **SHIPPED v1.500.** `delete State.players.lockerSlotToggle[pid]` in `onPlayerLeaveGameImpl`; rejoiner gets fresh `[2,2,2,2]` defaults. Lifecycle Map status flipped from ❌ to ✓.
4. **The v1.491 MP playtest with the 3 diagnostic toggles (Combat HUD / Vehicle Overlay / Spectator) is the dominant signal post-v1.497/v1.498.** With v1.497 S3 ship landed (vehicle deploy timer broadcast 200ms-coalesced) and v1.498 closing the long-match audit punchlist, the playtest now also serves as an S3-effectiveness measurement against a clean script-side baseline. If the spike persists with all three toggles off AND with S3 throttling shipped AND with all 5 confirmed leaks plugged, the driver is engine-side accumulation we cannot statically audit. **Note:** the script already restarts at match end, and the v1.491 spike manifests *within a single match* — so multi-match restart cadence is NOT a valid mitigation. The within-match scope means we need either (a) more aggressive intra-match resets (e.g. periodic `cleanupHudForPid` + lazy-rebuild), (b) reduction of accumulating engine surfaces (capture-sound queue back-pressure, world-log buffer churn, area-trigger churn), or (c) find another Tier S burst pattern whose conditions become more frequent as match state piles up (S1/S2 combat HUD coalesce is user-approved and pending; S4–S10 not yet ranked).

These leaks are wired into the reclaim ladder in [`conquest_optimization_analysis.md`](./conquest_optimization_analysis.md) Tier A as **A11** (L1, **SUPERSEDED by v1.496 dead-code removal**), **A12** (L3-L5 click trackers, **SHIPPED v1.500**), **A13** (L2 lockerSlotToggle, **SHIPPED v1.500**). **Long-match audit punchlist now closed.**

---

## Concurrent timer inventory (v1.497 audit, 2026-05-09)

**Why this section exists.** Static audit of every `Timers.setTimeout` / `Timers.setInterval` / `Clocks.CountDownClock` call site in `src/`, with peak concurrent count estimated per scenario. Surfaced as part of the v1.491 investigation: each pending Timer entry sits in `Timers.ACTIVE_IDS` (a `Set<number>` at [`foundation/bf6-utils/timers.ts:24`](../src/foundation/bf6-utils/timers.ts#L24)), and the engine schedules each via `mod.Wait(ms/1000)` — so ACTIVE_IDS size is a proxy for in-flight async work. Live measurement available via `Timers.getActiveTimerCount()` if needed.

**Scope:** `Timers.setInterval` is unused in `src/` — all recurring schedules use chained `setTimeout` (drift-resistant pattern). Each `Clocks.CountDownClock` instance internally chains its own `Timers.setTimeout` at [`foundation/bf6-utils/clocks.ts:134`](../src/foundation/bf6-utils/clocks.ts#L134), so a running clock = 1 active Timer entry.

### Timer call-site inventory (T1–T11)

| ID | Source | Per-fire window | Peak concurrent | Cancellable? |
|----|--------|-----------------|----------------|--------------|
| **T1** | [`index/player-join-leave.ts:155-156`](../src/index/player-join-leave.ts#L155) — staggered lazy-build (×2 per join) | 50 ms / 150 ms | 2 × concurrent joins (typical: 2; mass-join: ~10–16) | ❌ no token; fire-and-forget |
| **T2** | [`boundary/live-prebuild-scheduler.ts:52`](../src/boundary/live-prebuild-scheduler.ts#L52) — boundary prebuild batches | 0s, 1s, …, 9s post-LIVE | **10** at LIVE-start (drains over 9s) | ✅ token-based |
| **T3** | [`vanilla-spawner.ts:101`](../src/vehicles/vanilla-spawner.ts#L101) — DealDamage on rejected vehicles | 1,500 ms | up to **~16** at countdown reset | ❌ no cancel |
| **T4** | [`vanilla-spawner.ts:330`](../src/vehicles/vanilla-spawner.ts#L330) — spawn-dispatch race timeout | 3,000 ms | **1** (mutex serializes spawn dispatch) | ✅ resolves harmlessly to -1 |
| **T5** | [`delay-broadcast.ts:94`](../src/index/delay-broadcast.ts#L94) — startMatch broadcast schedule | until longest config delay (Firestorm: ~120s) | **4** (one per delay tunable) | ✅ token-based |
| **T6** | [`delay-broadcast.ts:119`](../src/index/delay-broadcast.ts#L119) — per-pid hide-timer after broadcast displays | 5,000 ms | up to **16** (one per visible pid) | ✅ stored on per-pid cache, replaceable |
| **T7** | [`supply-box-warm-scheduler.ts:87`](../src/interaction/supply-box-warm-scheduler.ts#L87) — warm queue drainer | ~2 s/pid sustained | **1** (single chained `_scheduledTimerId`) | ✅ token-based |
| **T8** | [`build-pacer.ts:91, 115`](../src/interaction/build-pacer.ts#L91) — paced builder drainer | 100 ms recurring while queue non-empty | **1** (single `_drainerTimerId`) | ✅ active-flag |
| **T9** | [`deploy-timer-ui.ts:1891`](../src/vehicles/deploy-timer-ui.ts#L1891) — S3 coalesce drain (v1.497) | 200 ms | **1** (single `vehicleDeployTimerHudCoalesceTimerId`) | ✅ via `clearVehicleDeployTimerHudCoalesceTimer` |
| **T10** | [`clock/state.ts:65,88`](../src/clock/state.ts#L65) — match clock (`Clocks.CountDownClock`) | 1 Hz tick, full match duration | **1** (single match clock instance) | ✅ via `.reset()` |
| **T11** | [`vanilla-spawner.ts:410`](../src/vehicles/vanilla-spawner.ts#L410) — per-slot respawn clock | until cooldown elapses (~30 s typical) | up to **~16** (one per vehicle slot in cooldown) | ✅ via `.reset()` |

### Three peak scenarios

**Peak A: Countdown-reset moment (just before LIVE).** Vehicle pool is being normalized — wrong-type spawns get teleported down + DealDamaged.
- T3 DealDamage: up to ~16 (one per rejected slot)
- T10 match clock: 1
- **Total: ~17**

**Peak B: LIVE-transition burst window (0–10s post-LIVE).** The startMatch storm.
- T2 boundary prebuild batches: 10
- T5 delay-broadcast schedule: 4
- T10 match clock: 1
- T8 build-pacer: 0–1
- **Total: ~15–16**

**Peak C: Mid-LIVE active combat — the v1.491 scenario.** This is where peak concurrent timers actually matters for the spike investigation.
- T11 respawn clocks: up to **16** (worst case: every slot simultaneously destroyed)
- T6 hide-timer after delay-broadcast: up to **16** (one per pid × concurrent broadcasts)
- T5 still-pending delay-broadcasts: 0–4
- T7 supply-box warm: 0–1
- T8 build-pacer drainer: 0–1
- T9 S3 coalesce: 0–1
- T1 staggered lazy-build (mass-join late-joiners): 0–4
- T10 match clock: 1
- **Realistic mid-combat sustained: ~20–25**
- **Absolute worst-case theoretical: ~40**

### Anomalies / observations

- **T1 and T3 have NO cancellation path.** If a player disconnects in the 50–150ms window post-join, the lazy-build timers fire into invalid state — `triggerLazyBuild` re-validates the pid, so safe, but the closure persists in `Timers.ACTIVE_IDS` until it drains. T3 is similar but wrapped in try/catch.
- **`Timers.getActiveTimerCount()` exists** at [`foundation/bf6-utils/timers.ts:98`](../src/foundation/bf6-utils/timers.ts#L98) — could be exposed via an admin diagnostic button for live measurement during an MP playtest.
- **The S3 ship (v1.497) added exactly 1 timer slot** (T9). Net peak-concurrent impact: +1 vs v1.496.

### Implications for v1.491

Concurrent timer count is unlikely to be the v1.491 driver on its own — 20–40 active timers is well within engine sandbox tolerance. **What matters is what fires per frame, not what's pending.** The respawn clocks (T11) and delay-broadcast hide-timers (T6) only do trivial work per fire (1 Hz tick paint, single visibility flip). Heavier per-fire work (S3 drain, build-pacer drain, supply-box warm) is single-instance.

If the diagnostic-toggle MP playtest is inconclusive, exposing `Timers.getActiveTimerCount()` on the admin panel as a read-only counter would let us see the actual peak during play. Documented here as a follow-up diagnostic option, not a current ship.
...
## Project Stats (v1.500)

**Bundle:** 926,137 / 1,048,576 bytes (11.67% headroom; +563 bytes from v1.499 — A12 added 3 cleanup helpers, A13 added 1 line, postbuild guardrail added zero bundle bytes). Long-match audit punchlist closed for real (5 of 5 leaks plugged). v1.500 also added a `scripts/postbuild.js` non-ASCII guardrail to prevent the silent-load failure class that broke v1.498. Comments are stripped at postbuild (verified: 0 matches for "Throttle state for" in `dist/bundle.ts`). Net delta v1.474 → v1.497: +59,860 bytes across three arcs:
...
- **Long-match accumulation + Tier S3 arc (v1.495–v1.497):** +464 bytes net. v1.495: +61 (A11 paired delete on `vehicleSpawnBaseTeamByObjId`, later superseded). v1.496: −361 (Tier C dead-code removal — `vehicleSpawnBaseTeamByObjId` was zero-readers; cache + writes + clearer + caller all deleted; A11 reclassified). v1.497: +825 (Tier S3 ship — `updateVehicleDeployTimerHudForAllPlayers` 200ms coalesce window via `Timers.setTimeout`; no new per-pid state). Lesson recorded: verify reader count before classifying a write site as a leak (A11 was incorrectly framed as leak; was Tier C dead code).
...
**v1.500 (2026-05-09) — Re-ship A12 + A13 with pure-ASCII inline content + postbuild non-ASCII guardrail (long-match audit punchlist actually closed).** Root cause of v1.498 silent-load failure identified: a single em-dash (U+2014) in an inline trailing comment on the `lockerSlotToggle` delete survived postbuild's comment-strip pass (which only strips `^// ...` line-leading comments, not inline `code; // note` trailing comments), and the Portal sandbox SILENTLY rejects scripts containing any non-ASCII byte (no console error, no event handlers register). v1.500 re-applies the same A12 + A13 functional code with: (1) **No inline comment on the lockerSlotToggle delete** — surrounding `delete State.players.armS[pid]`, `armG[pid]`, `armL[pid]` are uncommented; the new delete is self-documenting in context. (2) **Pure-ASCII docblocks** above each new helper (`-` instead of `—`, `disconnect-reconnect` instead of `disconnect→reconnect`). (3) **Postbuild guardrail** at [`scripts/postbuild.js`](../scripts/postbuild.js) step 11.5 hard-fails the build with `process.exit(1)` if any non-ASCII byte is found in `dist/bundle.ts`, printing line + character + code point. (4) **AGENTS.md notice** added as a top-level section "Non-ASCII Characters Are Banned in dist/bundle.ts" documenting the rule + common offenders + where it does and doesn't apply (`strings.json` exempt). (5) **Memory entry saved** at `~/.claude/projects/.../memory/feedback_portal_sandbox_ascii_only.md` so the lesson persists across conversations. Bundle delta: +563 bytes (925,574 → 926,137; -137 vs broken v1.498's 926,274 because the inline comment is gone).

**v1.499 (2026-05-09) — REVERT v1.498.** v1.498 attempt at A12 + A13 ("zero-risk" mechanical hygiene ship) broke script load at game start. v1.499 reverts all 3 helpers + 3 callsites + the lockerSlotToggle delete; bundle returns to byte-identical v1.497 functional state (925,574 bytes). Root cause not yet identified. Lesson recorded: even a change that exactly mirrors a proven-working symmetric pattern (`resetVehicleDeployPrimaryClickTrackerForPid` in vehicles/deploy-timer-ui.ts) is NOT automatically zero-risk just because the pattern is established. Investigation must precede any re-attempt — possibly bisect A12 vs A13 in isolation. **A12 and A13 are now BLOCKED in [`conquest_optimization_analysis.md`](./conquest_optimization_analysis.md) Tier A.**

**v1.498 (2026-05-09, REVERTED in v1.499) — Tier A12 + A13 hygiene ship: long-match audit punchlist closed.** Plugged 4 leaks identified in the v1.494 long-match accumulation audit. **A12** (3 click trackers L3-L5): added `resetAdminPanelPrimaryClickTrackerForPid` ([admin-panel/events.ts:7-11](../src/admin-panel/events.ts#L7)), `resetReadyDialogPrimaryClickTrackerForPid` ([interaction/ui-events-ready.ts:10-14](../src/interaction/ui-events-ready.ts#L10)), and `resetPlayerReadyPanelPrimaryClickTrackerForPid` ([interaction/ui-events-player-ready-panel.ts:32-36](../src/interaction/ui-events-player-ready-panel.ts#L32)) — exact mirror of the existing `resetVehicleDeployPrimaryClickTrackerForPid` pattern. All 3 wired into `cleanupHudForPid` ([index/player-join-leave.ts:77-79](../src/index/player-join-leave.ts#L77)) alongside the existing vehicle-deploy tracker cleanup. **A13** (locker slot toggle L2): added `delete State.players.lockerSlotToggle[pid]` to `onPlayerLeaveGameImpl`'s per-pid delete cluster ([index/player-join-leave.ts:203](../src/index/player-join-leave.ts#L203)), next to other arm-* deletes. Rejoiner gets fresh `[2,2,2,2]` (GadgetTwo per class) since server cannot associate rejoiner with previous preference (user direction 2026-05-09). **Net effect on v1.491:** zero direct (these leaks are within-match-irrelevant); the audit punchlist is now closed for completeness. Each leak was tiny per entry but technically unbounded across join/leave cycles. Bundle delta: +700 bytes (925,574 → 926,274). Lifecycle Map status flips from ❌ to ✓ for `lockerSlotToggle` and the 3 click trackers.

**v1.497 (2026-05-09) — Tier S3 ship: vehicle deploy timer HUD broadcast 200ms-coalesce throttle.** Replaced the body of `updateVehicleDeployTimerHudForAllPlayers` at [`vehicles/deploy-timer-ui.ts:1888`](../src/vehicles/deploy-timer-ui.ts#L1888) with a debouncing coalescer. 26 fire-and-forget broadcast call sites (HQ deploy state, vehicle spawn/destroy, vehicle enter/exit, match-flow transitions, ready-dialog countdown, admin toggle, player join/leave, plus an unconditional 1Hz `onLiveTick`) now collapse into one drain per 200ms window via `Timers.setTimeout`. Each call flips `vehicleDeployTimerHudDirty=true`; first call within an idle window schedules the drain; subsequent calls inside the window are O(1) flag-flips. At drain time, `forEachValidPlayer` runs once with the latest state (render plan computed at drain time, not flag-set time). Per-player path (`refreshVehicleDeployTimersForPlayerPreservingVisibility` + `lastRenderSignature` short-circuit) **unchanged**. New module-local state `vehicleDeployTimerHudDirty` + `vehicleDeployTimerHudCoalesceTimerId` + `VEHICLE_DEPLOY_TIMER_HUD_COALESCE_MS = 200`. New helper `clearVehicleDeployTimerHudCoalesceTimer()` cancels pending timer at game-mode init via `Timers.clearTimeout` (verified at [`foundation/bf6-utils/timers.ts:83`](../src/foundation/bf6-utils/timers.ts#L83) — silently ignores unknown IDs). Wired into [`index/game-mode.ts:55`](../src/index/game-mode.ts#L55) reset block. **Estimated CPU win:** ~5–10× fewer vehicle-HUD broadcasts in active 8–10p combat (from 5–15/sec to ≤5/sec; from ~1,280–3,840 slot-state computes/sec to ≤1,280/sec). Mirrors combat HUD's `TWL_CONQUEST_HUD_MAIN_UPDATE_SECONDS` throttle interval. User-approved 2026-05-09: timer doesn't need hyper-accurate frame-coincident updates; can catch up within reason. Bundle delta: +825 bytes (924,749 → 925,574). All 26 call sites unchanged. MP playtest with diagnostic toggles is the gate to confirm whether S3 was a meaningful contributor to the v1.491 1716ms-frame breach. Plan archive: [`5.09.26_conquest_S3_vehicle_deploy_timer_throttle_plan.md`](./5.09.26_conquest_S3_vehicle_deploy_timer_throttle_plan.md).

**v1.496 (2026-05-09) — Tier C dead-code removal: `vehicleSpawnBaseTeamByObjId` cache (supersedes v1.495 A11).** Post-v1.495 grep audit found ZERO read sites across `src/` for the cache that A11 had paired-deleted on destroy. The whole construct was write-only dead code: write at [`index/vehicle-events.ts:78`](../src/index/vehicle-events.ts#L78) on every vehicle spawn, plus the v1.495 destroy-side delete, plus a `clearSpawnBaseTeamCache` helper called once at game-mode init. None of those values were ever consumed. v1.496 deleted the entire construct: declaration in [`config/runtime.ts`](../src/config/runtime.ts), spawn-side write, v1.495 destroy-side delete, `clearSpawnBaseTeamCache` helper in [`vehicles/registration.ts`](../src/vehicles/registration.ts), and its lone caller in [`index/game-mode.ts`](../src/index/game-mode.ts). Bundle delta: −361 bytes (925,110 → 924,749). **Lesson recorded:** verify reader count via grep before classifying a write site as a leak — A11's "leak fix" framing was wrong because GC would have collected entries at game-mode init anyway (the fix paired a delete against a dead cache). The right fix was Tier C deletion of the cache entirely, which v1.496 does. A11 row in `conquest_optimization_analysis.md` marked SUPERSEDED.

**v1.495 (2026-05-09) — A11 (long-match audit L1, since superseded by v1.496).** Added paired `delete vehicleSpawnBaseTeamByObjId[getObjId(eventVehicle)]` to `onVehicleDestroyedImpl` at [`index/vehicle-events.ts:106`](../src/index/vehicle-events.ts#L106), mirroring the unconditional spawn-side write at line 78. Initially framed as a Tier A leak fix targeting the highest-growth-rate confirmed leak from the long-match accumulation audit (~240 entries / 20-min match). Bundle delta: +61 bytes. **Superseded by v1.496** after a follow-up grep found zero readers, reclassifying the cache as Tier C dead code. The v1.495 line of code was deleted alongside the rest of the construct in v1.496. Retained in changelog for historical accuracy + as the lesson archive.
...
Module: Firestorm helicopter deploy/live timer display with direct spawn buttons (mega-file ~2,099 lines post-S3; 59 internal helpers). External callers use `updateVehicleDeployTimerHudForAllPlayers` / `prebuildVehicleDeployTimerHudHiddenForPlayer` / `revealVehicleDeployTimerHudForPlayer` / `buildVehicleDeployTimerRenderPlan` / `applyVehicleDeployTimerRenderPlanContent`. Owns largest per-pid widget cache (M1). v1.497: `updateVehicleDeployTimerHudForAllPlayers` now throttles via 200ms coalescing (Tier S3 ship); module-local state `vehicleDeployTimerHudDirty` + `vehicleDeployTimerHudCoalesceTimerId` + `VEHICLE_DEPLOY_TIMER_HUD_COALESCE_MS=200` declared near top; new helper `clearVehicleDeployTimerHudCoalesceTimer()` (1 — called from `index/game-mode.ts:55`) cancels any pending drain at game-mode init. Per-player path (`refreshVehicleDeployTimersForPlayerPreservingVisibility`) unchanged.
...
| `lockerSlotToggle[pid]` | First locker open | Paired delete in `onPlayerLeaveGameImpl` per-pid delete cluster (added v1.500, A13; v1.498 attempt broke load via em-dash in inline comment, reverted in v1.499, re-shipped clean in v1.500). Rejoiner gets fresh `[2,2,2,2]` defaults per user direction 2026-05-09. | ✓ (v1.500 fix) |
...
- ~~**`State.players.lockerSlotToggle[pid]`** — no leave-time delete~~ **RESOLVED v1.498 (A13).** User confirmed 2026-05-09 that the server cannot associate a rejoiner with their previous preference, so `delete` on leave is correct; rejoiner gets fresh `[2,2,2,2]` defaults. Status flipped from ❌ to ✓.
