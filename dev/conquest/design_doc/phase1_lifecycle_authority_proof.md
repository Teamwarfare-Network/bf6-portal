...
Requirement format: `lifecycle mutator inventory -> owner function -> allowed callers -> guard/latch proof -> blocked legacy path proof`
...
- The full 5-state conquest lifecycle (`NOT_READY -> PRE_MATCH -> LIVE_MATCH -> POST_MATCH -> RESET`) remains a later cutover task per design rules.
