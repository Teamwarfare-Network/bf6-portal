...
function buildAdminPanelWidgets(eventPlayer: mod.Player, adminContainer: mod.UIWidget, playerId: number): void {
    ...
    if (FEATURE_MIN_SPAWN_TEST) {
        // v1.236 Phase 1 diagnostic — five buttons on one compact row cycling through teleport-variant
        // combinations of the isolated F16 spawn+seat pattern. Labels (BASE/S-1/TP0/TP5/TPW) come from
        // strings.json adminPanel.tester.buttons.minSpawn*. Handlers live in admin-panel/events.ts and
        // call runMinimalSpawnTest with a matching MinimalSpawnTestVariant.
        const minSpawnRowY = row0Y + (buttonSizeY + rowSpacingY) * 10;
        const minSpawnRowHeight = 28;
        const minSpawnGapX = 6;
        const minSpawnRowContentWidth = (buttonSizeX + 8 + labelSizeX + 8 + buttonSizeX);
        const minSpawnButtonWidth = Math.floor((minSpawnRowContentWidth - (minSpawnGapX * 4)) / 5);
        addMinSpawnTestButton(eventPlayer, adminContainer, playerId,
            testerBaseX + ((minSpawnButtonWidth + minSpawnGapX) * 0),
            minSpawnRowY, minSpawnButtonWidth, minSpawnRowHeight,
            UI_TEST_BUTTON_MIN_SPAWN_F16_ID, UI_TEST_MIN_SPAWN_F16_TEXT_ID,
            mod.stringkeys.twl.adminPanel.tester.buttons.minSpawnBase);
        addMinSpawnTestButton(eventPlayer, adminContainer, playerId,
            testerBaseX + ((minSpawnButtonWidth + minSpawnGapX) * 1),
            minSpawnRowY, minSpawnButtonWidth, minSpawnRowHeight,
            UI_TEST_BUTTON_MIN_SPAWN_AH64_ID, UI_TEST_MIN_SPAWN_AH64_TEXT_ID,
            mod.stringkeys.twl.adminPanel.tester.buttons.minSpawnSeatMinusOne);
        addMinSpawnTestButton(eventPlayer, adminContainer, playerId,
            testerBaseX + ((minSpawnButtonWidth + minSpawnGapX) * 2),
            minSpawnRowY, minSpawnButtonWidth, minSpawnRowHeight,
            UI_TEST_BUTTON_MIN_SPAWN_F22_ID, UI_TEST_MIN_SPAWN_F22_TEXT_ID,
            mod.stringkeys.twl.adminPanel.tester.buttons.minSpawnTpGround);
        addMinSpawnTestButton(eventPlayer, adminContainer, playerId,
            testerBaseX + ((minSpawnButtonWidth + minSpawnGapX) * 3),
            minSpawnRowY, minSpawnButtonWidth, minSpawnRowHeight,
            UI_TEST_BUTTON_MIN_SPAWN_MH6_ID, UI_TEST_MIN_SPAWN_MH6_TEXT_ID,
            mod.stringkeys.twl.adminPanel.tester.buttons.minSpawnTp5y);
        addMinSpawnTestButton(eventPlayer, adminContainer, playerId,
            testerBaseX + ((minSpawnButtonWidth + minSpawnGapX) * 4),
            minSpawnRowY, minSpawnButtonWidth, minSpawnRowHeight,
            UI_TEST_BUTTON_MIN_SPAWN_ABRAMS_ID, UI_TEST_MIN_SPAWN_ABRAMS_TEXT_ID,
            mod.stringkeys.twl.adminPanel.tester.buttons.minSpawnTp5yLong);
        addMinSpawnTestButton(eventPlayer, adminContainer, playerId,
            testerBaseX,
            minSpawnRowY + minSpawnRowHeight + 4, minSpawnButtonWidth, minSpawnRowHeight,
            UI_TEST_BUTTON_MIN_SPAWN_SPWN_ID, UI_TEST_MIN_SPAWN_SPWN_TEXT_ID,
            mod.stringkeys.twl.adminPanel.tester.buttons.minSpawnSpwn);
    }

    syncAdminMatchLengthLabelForAllPlayers();
}

// v1.235 Phase 1 — compact admin-panel button for the Test Minimal Spawn row.
// Uses the generic `{0}` string key so dynamic labels don't need Strings.json edits.
function addMinSpawnTestButton(
    ...
    const parent = safeFind(buttonId + "_BORDER") ?? containerBase;
    const label = addCenteredButtonText(
        labelId,
        width,
        height,
    ...
    if (label) mod.SetUITextColor(label, ADMIN_PANEL_BUTTON_TEXT_COLOR);
}
