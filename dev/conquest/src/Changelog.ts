...
// Ship C: sound/VO recipient snapshot caching. flushCaptureSoundQueue + flushCaptureVoiceOverQueue now snapshot mod.AllPlayers() once per flush and pass to each recipient-resolver instead of calling it per event. Reduces ~100-200 engine array allocations per flush at 10p to 1. Recipient helpers accept optional allPlayers/allPlayerCount params with fallback to fresh AllPlayers() call for backward compat on any future call sites. Bundle delta: TBD.
// Phase 2 (Ship B): hot-path combat HUD engine-call reduction -- direct-write helpers + diff-cache. New hud-core/direct-write.ts skips resolveLiveUITextWidget per-frame re-validation (~900 engine calls/frame at 10p). render.ts now diffs every widget value against entry.lastSnapshot before writing -- skips unchanged labels/colors/vis. At steady state ~70-90% of widget writes are to unchanged values; diff-cache collapses them. Diff compares serializable keys (numeric tickets, string labels) not mod.Message object refs. Single-threaded engine guarantees handle validity within a frame; per-frame stale-entry cleanup + generation-stamp rebuild handle edge cases. Bundle delta: TBD.
// Ship A: S1/S2 combat HUD broadcast coalesce (conquest_e_refactor branch). Replaces 5 force=true synchronous broadcast sites with a 50ms Timers.setTimeout debounce (mirrors v1.497 S3 vehicle deploy timer coalesce pattern). Sites: area-triggers enter/exit (index/area-triggers.ts), capture lost/captured (index/capture-tickets.ts), spawn-charge deploy (state/spawn-charge.ts). Each now calls markHudDirty() + scheduleCombatHudCoalesceDrain() instead of updateConquestCombatHudForAllPlayers(true). The drain runs updateConquestCombatHudForAllPlayers() without force -- respects the existing 200ms main throttle. Collapses N forced broadcasts per frame into 1 per 50ms window. New module: src/hud/combat-hud-coalesce.ts (40 lines). Admin-panel toggle force=true preserved intentionally. Game-mode init wires clearCombatHudCoalesceTimer(). Cleanup at index/game-mode.ts:56. All ASCII, full-line comments (stripped by postbuild). Bundle delta: TBD.
// v1.514: Gate delay-broadcast elapsed-time activation messages on the same feature-enable flags the pregame countdown UI uses. If a feature is disabled at match start, its activation broadcast (e.g., 'Aircraft can now HQ Ground Deploy!' at +30s) no longer fires -- previously fired unconditionally based on the map config's delay tunable, even when the feature itself was off. Gates added inline at the schedule call sites in DelayBroadcast.scheduleDelayBroadcastsForLive matching the ready-dialog/countdown-flow.ts:108-118 pattern: (1) HQ broadcast -> vehicleDeployMethod >= VEHICLE_DEPLOY_METHOD_HQ, (2) Air-deploy broadcast -> airDeployEnabled, (3) Forward-deploy broadcast -> forwardDeployEnabled, (4) Gadgets broadcast -> isSupplyBoxesEnabled(). Reads State.round.modeConfig.confirmed.* values, which are locked at LIVE start so a single read at startMatch is sufficient. Per-feature timer simply isn't scheduled when the gate fails -- no scheduled callback, no token check, no broadcast. ASCII-clean. Postbuild guardrail clean. Bundle 930,309 / 1,048,576 (11.32% headroom).
// v1.513: Revert v1.512 grace flag. Per user direction 2026-05-12: spec cycle should give exactly ONE free deploy (the immediate click-to-deploy after exit), not two. Removed spectatorExitGraceByPid field from State.conquest.spawnCharge type, init from runtime-state.ts + conquest-scaffold.ts + spawn-charge.ts resetSpawnChargeState, per-pid clear from clearPidSessionState, consume check from onPlayerDeployedSpawnCharge, and the flag-set from exitSpectatorMode. Behavior matches v1.510/v1.511: enter spec free, exit spec free, first deploy after exit free via 'spectator_transition' reason, all subsequent deploys (including first death-respawn) charged normally. The defensive markNextDeployReason in enterSpectatorMode stays per user direction (covers the unlikely case ForcePlayerExitVehicle or teleport triggers a spurious deploy event). User notes will revise this rule once a spectator button is added to the deploy screen. ASCII-clean. Postbuild guardrail clean.
// v1.512: Spectator exit grace: first death-respawn after spec exit is also ticket-free. v1.510/v1.511 made the IMMEDIATE post-exit click-to-deploy free via the 'spectator_transition' reason; this ship adds a one-shot grace flag so the FIRST non-exempt deploy AFTER that initial post-exit deploy is also exempted. Rationale (user 2026-05-11): the only way to re-enter spectator currently is by spawning in and choosing it from a HUD menu, so the player needs at least one free death-respawn after the spec-exit cycle for re-spectating to cost zero tickets. Will revise if a spectator button gets added to the deploy screen. Implementation: new State.conquest.spawnCharge.spectatorExitGraceByPid: Record<number, boolean> field added to runtime-types.ts; init'd in runtime-state.ts initial state + index/conquest-scaffold.ts round-init + spawn-charge.ts resetSpawnChargeState; cleared per-pid in clearPidSessionState. Set to true in exitSpectatorMode immediately after the existing markNextDeployReason call. Consumed in onPlayerDeployedSpawnCharge AFTER the exempt-reason check (so the immediate spectator_transition deploy doesn't waste the grace) but before the dup-check + charge logic. Net behavior: spec entry/exit cycle now grants the player TWO free deploys -- (1) immediate post-exit deploy via reason exempt, and (2) the first death-respawn after that via the grace flag. Subsequent deploys charge as normal. ASCII-clean. Postbuild guardrail clean.
// v1.511: v1.510 follow-up: complete the spectator_transition counter wiring across the two remaining literal-object init sites that bundle-level typecheck flagged. v1.510 added the new reason to ConquestSpawnChargeReason + newReasonCounterState + getReasonCode + getReasonCounterTotal + the exempt list, but missed the two direct counter-object literals that don't go through newReasonCounterState: src/index/conquest-scaffold.ts (reset on round-init x2) and src/state/runtime-state.ts (initial state x2). Both reset paths now include 'spectator_transition: 0'. Bundle typecheck clean after rebuild. No behavior change vs v1.510's ticket-exemption logic (counter totals only matter for debug-print snapshots; the exempt-list check at onPlayerDeployedSpawnCharge is unaffected); this brings the bundle into type-strict consistency.
// v1.510: Spectator bug fixes: invulnerability while spectating + spec-transition redeploys are ticket-free. Bug 1 (self-damage): mod.SetPlayerIncomingDamageFactor(eventPlayer, 0) on enterSpectatorMode after slot claim; restore to 1 on exitSpectatorMode (paired) and on onSpectatorMatchEnd / onSpectatorFreshSetup (defensive against carry-over). Spec body sits in the Y=-500 hide-room but engine-applied damage events (e.g., grenade splash from a throw immediately before claim, in-flight projectile) could still kill it; factor=0 makes the body unkillable for the duration of the slot. Bug 2 (spec transition shouldn't cost tickets): added 'spectator_transition' to ConquestSpawnChargeReason union; wired into newReasonCounterState + getReasonCode (8) + getReasonCounterTotal in spawn-charge.ts; added to the early-return exempt list in onPlayerDeployedSpawnCharge alongside vehicle_deploy and team_switch. enterSpectatorMode marks the reason defensively in case ForcePlayerExitVehicle triggers a spurious deploy event; exitSpectatorMode marks the reason BEFORE UndeployPlayer so the player's next click-to-deploy after returning to the deploy screen is exempted. Net behavior: spec entry/exit cycle costs zero tickets; subsequent normal deploys remain charged as 'deploy'. PCT reference checked - it has no explicit invulnerability mechanism, SDK SetPlayerIncomingDamageFactor is the canonical Portal API. Both fixes ASCII-clean. Postbuild guardrail clean.
// v1.509: Default game mode preset: 10v10 → 12v12. Changed READY_DIALOG_GAME_MODE_DEFAULT_INDEX in foundation/gameplay.ts from 1 to 2 (the index into READY_DIALOG_GAME_MODE_OPTIONS = [8v8, 10v10, 12v12, 16v16]). The constant seeds initial game mode in 3 runtime-state init paths + 4 reset/refresh call sites in map-runtime.ts and ready-dialog/mode-config-presets.ts; all flip together. Existing 'TWL Conquest 12v12' preset package authored on Operation Firestorm (config/maps/operation-firestorm.ts:237) provides the players/vehicle layout. Single-character source change. ASCII-clean.
// v1.507: Document header re-injection + strip-from-bundle marker convention in AGENTS.md. New subsections under Comment Bundle-Stripping Behavior: (1) 'Header re-injection (postbuild step 11)' documents the v1.502 mechanism that re-prepends src/header-file.ts to the top of dist/bundle.ts after the comment-strip pass; explains the targeted strips it runs (@ts-nocheck, // Module:, // *policy) before prepending. (2) 'Strip-from-bundle marker: // *<keyword>' formalizes the v1.504 convention - lines starting with // * followed by a registered keyword in src/header-file.ts are stripped from the bundle, used for source-only project conventions like versioning policies that shouldn't ship to the engine. Currently only // *policy is registered; new keywords require editing postbuild step 11. Documentation-only ship - no code change, no bundle size change (AGENTS.md is not bundled).
// v1.504: Strip lines starting with '// *policy' from prepended header content. The user marks source-only project conventions (versioning rules, file-policy notes) with a '// *policy' prefix in src/header-file.ts; postbuild now removes these lines from the bundle before the header is prepended at step 11. Pattern: ^[ \\t]*//\\s*\\*policy[^\\n]*\\n. Bundle 929,100 bytes (-940 from v1.503/v1.502 baseline thanks to 7 policy lines stripped). Header in dist/bundle.ts now shows: version line + WIP note + license + authors/attribution, without the policy-rule documentation. ASCII-clean confirmed (postbuild guardrail). Updated header content (license consolidated to a tighter block; new attribution entry for Portal Cinematic Camera by Nodone) folded in via this same ship since the source file was edited directly by the user.
// v1.502: Restore full header block (versioning / license / attribution) at the top of dist/bundle.ts. Postbuild step 11 was previously prepending only the single 'version: ...' line; now prepends the entire header-file.ts content (after stripping @ts-nocheck and // Module: build-time directives), matching the modlib incorporation pattern. Header content survives because it's prepended AFTER the comment-strip pass. Also cleaned up the literal-BOM regex on line 255 of postbuild.js to use the \\uFEFF escape pattern matching the existing 3 BOM-strip sites in that file. Bundle 930,040 / 1,048,576 (+3,903 bytes from v1.501; 11.31% headroom). Header-file.ts source is pure ASCII (verified); postbuild non-ASCII guardrail clean. License + attribution + versioning block now visible to anyone reading dist/bundle.ts.
// v1.501: User-facing string rename: Admin Panel surface renamed to Debug Panel + tester header banner removed. Renamed twl.adminPanel.buttons.panel from 'Admin Panel' to 'Debug Panel' (the toggle button label that opens the surface). Also renamed the audit/log strings: twl.adminPanel.accessed (now '{0} accessed the Debug Panel'), twl.adminPanel.actionPressed (now '{0} pressed {1} in the debug panel'), twl.adminPanel.actionCountVictoryFormat (now 'Debug Panel Buttons used {0} times'). Cleared twl.adminPanel.tester.header from 't1 = left team; t2 = right team' to '' so the developer-facing banner at the top of the panel renders blank (text widget retained for layout symmetry; pre-existing 22px gap above row 0 unchanged). Role-related strings (Game Admin, Claim Admin, Give Up Admin, admin hint text) preserved since those describe the role, not the panel surface. Pure-ASCII content, postbuild guardrail clean. User-approved 2026-05-09.
// v1.500: Re-ship A12 + A13 with pure-ASCII inline content + add postbuild non-ASCII guardrail. v1.498 broke script load silently via a single em-dash (U+2014) in an inline trailing comment after the lockerSlotToggle delete; postbuild's comment-strip pass only removes line-leading // comments not inline ones, so the em-dash survived into dist/bundle.ts and the Portal sandbox silently rejected the entire script (no console error, no event handlers registered). v1.499 reverted; v1.500 re-applies the same code with: (a) no inline comment on the lockerSlotToggle delete (surrounding deletes are uncommented and self-documenting); (b) full-line // docblocks above each new helper using ASCII-only chars; (c) a new postbuild guardrail at scripts/postbuild.js step 11.5 that hard-fails the build with process.exit(1) if any non-ASCII byte is found in dist/bundle.ts, printing offending line + character + code point. Also documented the rule in AGENTS.md as a new top-level section Non-ASCII Characters Are Banned in dist/bundle.ts. Bundle 926137 / 1048576 (11.67% headroom). All A12 + A13 functional content identical to v1.498 except the inline comment removal.
// v1.499: REVERT v1.498 (A12 + A13 hygiene ship): broke script load at game start. Removing 3 reset*PrimaryClickTrackerForPid helpers (admin-panel/events.ts, interaction/ui-events-ready.ts, interaction/ui-events-player-ready-panel.ts) + their 3 callsites in cleanupHudForPid + the State.players.lockerSlotToggle[pid] delete in onPlayerLeaveGameImpl. Returning to v1.497 functional state. Root cause not yet identified — will investigate with caution before re-attempting. Apologies for the broken zero-risk claim; the change pattern was symmetric with proven existing helpers but something in the implementation broke script load.
// v1.498: Tier A12 + A13 hygiene ship: plug 4 long-match-audit leaks. A12 adds 3 cleanup helpers (resetAdminPanelPrimaryClickTrackerForPid, resetReadyDialogPrimaryClickTrackerForPid, resetPlayerReadyPanelPrimaryClickTrackerForPid) mirroring the existing resetVehicleDeployPrimaryClickTrackerForPid pattern; all 3 called from cleanupHudForPid alongside the vehicle-deploy tracker cleanup. A13 adds delete State.players.lockerSlotToggle[pid] to onPlayerLeaveGameImpl per-pid delete cluster; rejoiner gets fresh [2,2,2,2] GadgetTwo defaults per user direction 2026-05-09 (server cannot associate rejoiner with previous preference). Closes the long-match audit punchlist (L1 dead-code-deleted v1.496; L2-L5 plugged here). Each leak was tiny per entry but technically unbounded across join/leave cycles. Zero risk: paired-deletion pattern is symmetric with proven existing tracker cleanup.
// v1.497: Tier S3 ship: throttle updateVehicleDeployTimerHudForAllPlayers via 200ms coalesce window. Replaces 26 fire-and-forget broadcast call sites' synchronous fan-out with a single debounced fire. Each call sets a dirty flag; first call schedules a Timers.setTimeout 200ms drain; subsequent calls within the window are no-ops; at drain time forEachValidPlayer runs once and the dirty flag is consumed. Cuts vehicle HUD broadcast frequency by ~5-10x in active 8-10p combat (from 5-15 broadcasts/sec to <=5/sec). Per-player render path (refreshVehicleDeployTimersForPlayerPreservingVisibility + lastRenderSignature dedupe) unchanged. User-approved 2026-05-09 (200ms window mirrors combat HUD throttle; no force-bypass parameter); ships alone for clean diagnostic-toggle MP bisection attribution. Game-mode init clears any pending coalesce timer (clearVehicleDeployTimerHudCoalesceTimer in vehicles/deploy-timer-ui.ts called from index/game-mode.ts reset block).
// v1.496: Tier C dead-code removal: drop the entirely write-only vehicleSpawnBaseTeamByObjId cache (zero readers across src/). Removes the declaration in config/runtime.ts, the spawn-side write in index/vehicle-events.ts, the v1.495 destroy-side delete in vehicle-events.ts, the clearSpawnBaseTeamCache helper in vehicles/registration.ts, and its lone caller in index/game-mode.ts. Supersedes A11 (v1.495) — that fix paired a delete against a dead cache, so it reclaimed nothing observable. The right fix is to delete the cache entirely, which this ship does. The 'long-match audit L1' line in optimization docs should now be marked dead-code-deleted rather than leak-fixed.
// v1.495: A11 (long-match audit L1): plug vehicleSpawnBaseTeamByObjId leak by adding paired delete in onVehicleDestroyedImpl. Cache was previously only cleared at game-mode init; now mirrors the spawn-side write at line 78 so the per-vehicle team binding is reclaimed on every destroy event. Smallest of the confirmed long-match accumulation leaks (~240 entries leaked per 20-min match pre-fix); shipped first because it was the cheapest mechanical fix while diagnostic playtest results are pending.
...
// v1.483: Spectator hide-room pinned to absolute Y=-500 (no more sky obstacles for aircraft); free-cam horizontal gain 1.0->3.0, vertical speed 8.0->20.0; pitch inverted so mouse-up tilts up.
// v1.482: Spectator Ship 3 typecheck fix: drop explicit mod.FixedCamera type annotation (missing from vendored bf6-portal-mod-types, same gap as GetFixedCamera). Use 'any' for the camera handle. v1.481 cmd /c npx tsc passed silently but & npx tsc surfaced TS2694.
// v1.481: Spectator Ship 3 (Free Camera, PCT pattern): WASD-driven free-fly camera control. Per-tick mod.SetObjectTransform loop animates the same Godot FixedCamera while spectator slot is held. Reads LinearVelocity decomposed against FacingDirection for horizontal motion; Jump/Crouch for vertical (+/-Y); Sprint for x2.5 speed multiplier. Reduced SPECTATOR_LOCKED_INPUTS to PCT free-cam set (kept weapon/slot/interact restrictions, released movement/sprint/jump/crouch). Module-local _spectatorFreeCameraLoopToken provides one-tick cancellation; cleanup hooks bump the token. Methodology adapted from PCT StartFreeCamera (reference_implementations/reference_nodone_cinematic_camera/main_module.ts:4428); original implementation, empirical smoothing/clamp/sprint constants reused as facts.
// v1.480: Spectator hide-room: cast (mod as any).GetFixedCamera to bypass vendored bf6-portal-mod-types gap (same pattern as Cameras.Fixed cast). v1.479 emitted with TS2339 typecheck failure on dist/bundle.ts because @ts-nocheck doesn't survive bundle concatenation; the cmd /c invocation in the post-bump check was swallowing the error.
// v1.479: Spectator Ship 2 follow-up: input restriction (body was still walking) + PCT-pattern sealed underground hide-room (4 walls + floor + ceiling at camPos+(0,-50,0)) + sky-spawn fallback on water hit + DisplayNotificationMessage self-confirmation. Body parked at HQ was visible to teammates as motionless soldier; now teleported into invisible sealed cube. Despawn on match-end / disconnect / fresh-setup. Methodology adapted from reference_implementations/reference_nodone_cinematic_camera SpawnDirectorControlRoom; original implementation.
// v1.478: Spectator Ship 2 (Mitigation C, HUD-persistence): single-spectator-slot integration. New State.players.spectatorPid scalar; new spectator-action.ts (enterSpectatorMode + lifecycle hooks) + coach-button-sync.ts (per-pid panel/dialog button state). COACH wired on Player Ready Panel + admin's full ready dialog when MapConfig.spectatorCameraId is authored (Firestorm only). Click attaches Cameras.Fixed directly (player already deployed pre-match), keeps combat HUD so spectator watches tickets/flags/captures. Boundary skip for spectator pid; defensive OnPlayerDeployed re-attach. Filtered out of getActivePlayers. Lifecycle cleanup on disconnect/match-end/fresh-setup. New string playerSpectating (approved 2026-05-08). Probe.ts deleted.
// v1.477: Spectator probe HUD: pass cameraId as number (was literal string template producing 'unknown string')
// v1.476: spectator probe Ship 1 typecheck fix: mod.Cameras.Fixed is present in SDK 1.2.3 (enum value 1) but missing from the project's vendored bf6-portal-mod-types. v1.475 emitted to dist with a TS2339 typecheck failure on the bundled file (source @ts-nocheck doesn't carry through to the concatenated dist/bundle.ts). Fixed via CAMERAS_FIXED = (mod.Cameras as any).Fixed as mod.Cameras pattern, mirroring foundation/gameplay.ts:204's precedent for stale-vendored-types enum values. Runtime acceptance of Cameras.Fixed remains the actual probe target.
// v1.475: spectator camera Ship 1 (probe). New src/spectator/probe.ts wires Player Ready Panel COACH button on maps that author MapConfig.spectatorCameraId; click runs runSpectatorProbe -> mod.UndeployPlayer + mod.SetCameraTypeForPlayer(Cameras.Fixed, id) + 12s debug HUD overlay confirming the click fired. Probes I-4 (Cameras.Fixed runtime acceptance on undeployed player) + I-12 (engine spectator UI overlay competing with Fixed camera). Operation Firestorm gets cam ObjId 667 (FixedCameraSpectator at -83.80, 246.37, -72.91 in MP_TWL_Conquest18_FireStorm.spatial.json). Maps without the field retain the original Wave 4 D3 disabled-grey COACH treatment. NO slot tracking, ready-marking, broadcasts, or lifecycle cleanup -- those land in Ship 2 after the probe resolves. Plan archive: design_doc/5.08.26_conquest_spectator_cam_plan.md.
// v1.474: Late-join during countdown fix (CQ_Bug_115): remove areAllActivePlayersReady() check from isPregameCountdownStillValid. A fresh late-joiner's undefined readyByPid value was failing this check every animation frame, cancelling the countdown via a bail path that leaks mod.EnableAllPlayerDeploy(false). Result: all watching players were locked out on the deploy screen post-cancel. The 'ready' concept no longer applies once the countdown has begun -- only admin reset (token mismatch) or match-end can abort the countdown now. Plan: design_doc/5.07.26_late_join_during_countdown_fix_plan.md.
// v1.473: Add Operation Firestorm yellow-smoke supply boxes 1058 (T1 mid-map at -462.743/138.347/37.2363) and 1059 (T2 mid-map at 339.316/113.847/-110.546). Match the existing flag-side supply box pattern (1050-1055): ownerTeamId 0, VFX_YELLOW_SMOKE, no disableOnLive flag (available pre-game and during LIVE alike). Spatial source: MP_TWL_Conquest17_FireStorm.spatial.json with matched WorldIcon/InteractPoint ObjId pairing.
// v1.471: Late-join crash defense (v1.469 server-crash mitigation): (Phase A) outer try/catch wrapper on async OnPlayerJoinGame export -- silent script-runtime kill on unhandled rejection during late-join becomes a logged console error instead. (Phase B) vehicleDeployTimerCache[pid] delete relocated from inside resetUiForPlayerOnJoin (post-await) to onPlayerJoinGameImpl sync prelude (pre-await) -- closes a cache-mutation race against runRoundStartDelayHudLoop where a tick-driven builder could create a late-joiner cache entry between engine-bind and T=0, get it deleted at T=100ms, then re-bound by the lazy-build cohort against engine-side widgets still tied to the freshly-deleted cache record. (Phase C) post-await idempotent re-read of perspectiveTeamByPid -- if the T=0 safeGetTeamNumberFromPlayer returned 0 (engine team-bind lag for late-joiners), re-read after the 100ms wait so downstream HUD code sees a valid team. MP repro validation pending; SP smoke validates no-regression on cold-launch single-player join.
...
// v1.450: Conquest HUD backplates v2 polish: team-name backplates lose vertical padding so they match the tickets backplate height (28) for visual alignment in the top HUD row. Engage status backplate shrunk via dedicated ENGAGE_STATUS_BOX_* constants — width 110 (just barely fits NEUTRALIZING at fontSize 18) + height 22, centered inside engage root. Eliminates the wide-bar visual that overlapped the chevron / ticket-diff area in v1.449.
// v1.449: Conquest HUD: backplates added behind team names + engage status text. Reuses tickets-box visual style (Blur fill, dark color, 0.75 alpha) per user direction. +3 widgets/pid in M3 (~92 -> ~95). Visibility toggled in lockstep with the text widgets via render.ts + lifecycle.ts.
// v1.448: CQ_Bug_94 follow-up: drop isSlotEmpty + slot-targeted RemoveEquipment from non-Engineer give helpers (giveMedicSmoke, giveAssaultItem, giveReconItem). isSlotEmpty was the remaining GetInventoryAmmo source on placement paths. AddEquipment clobbers cleanly. Dup prevention via HasEquipment + lockerSlots gray-out remains.
// v1.447: CQ_Bug_94 — supply-box menu open no longer emits 4 GetInventoryAmmo invalid-item engine errors per interaction on Medic/Assault/Recon. Per-class HasEquipment-based probes (probeAssaultSlot, probeMedicSlot, probeReconSlot) + dispatcher route non-Engineer classes through HasEquipment-only path. Engineer probeSlot untouched. See 5.03.26_conquest_supplybox_medic_fix_plan.md.
// v1.446: Vehicle deploy timer bars: added 'WAIT' label drawn on top of the red/gray bar (black text, centered, visible only in timer mode). New twl.ui.wait string key.
// v1.445: CQ_Bug_58 — pre-game ready state now persists through death + leaving main base; only team switch + admin config change auto-unready. See 5.02.26_conquest_ready_tuning_plan.md.
// v1.444: Wave 6 chevron polish: invert bleed chevron colors (left=red on blue bar, right=blue on red bar). Improves contrast post-shadow-removal + reinforces enemy-bleeding semantic.
// v1.443: Wave 6 Ship 0+1c+1d: maxPasses 128->4 disconnect speedup; eliminate combat HUD compass shadow rings (~280 widgets/pid reclaim); stagger 3 join triggers across 3 frames
// v1.442: CQ_Bug_57 fix: squad-spawn on teammate at HQ no longer flags spawner as out-of-bounds. tryInheritZonesFromNearbyTeammate in boundary/enforcement.ts now propagates inOwnHQ from the squad-spawn target (was explicitly omitted at v0.1 inheritance scope, on the assumption the anchor-radius probe would always catch at-HQ spawns; in practice the probe can fail when position isn't yet settled at deploy or when the HQ trigger volume is wider than DEPLOY_MAIN_BASE_RADIUS_METERS). Pre-fix: spawner inherited all-four-false flags from a teammate at HQ -> classifier fired prelive_main_base or ground_combat_zone violation. Fix: one-line addition state.inOwnHQ = teammateState.inOwnHQ || state.inOwnHQ. Edge: 25m proximity could over-inherit if teammate is at HQ-edge and spawner lands just outside, but trigger-exit event corrects on next physical movement -- acceptable trade vs always-flag-as-violation.
// v1.441: Wave 5 Ship 1 polish v2: align bar border to the plate's exact coords + size per user feedback. Pre-fix: border was inset 2px from the root edges (50x14 inside the 54x18 root) which left a slim plate margin visible around the gray border. Fix: border now sits at root-local (0, plateOffsetY) with plateWidth x plateHeight (perfectly overlays the plate); fill is 1px inset inside the border. Visual result: gray bar fully fills the box behind it; red fill is properly spaced inside the gray bar.
// v1.440: Wave 5 Ship 1 polish: bar geometry fix per user feedback. Pre-fix: bar used digitLayoutWidth (44px) for width and CenterLeft/Center anchor combo that put fill outside parent (visibly tiny + misaligned red bar inside the gray plate). Fix: switch to TopLeft anchors with explicit pixel positions. Border now insets 2px from root edges (~50x14 in the 54x18 root); fill insets 1px inside border (~48x12 max). Bar fills the plate area visually. Cache now stores barFillMaxWidth + barFillHeight so per-tick update path is geometry-config-independent (drops the dead FALLBACK_BAR_GEOMETRY_CFG hack).
// v1.439: Wave 5 Ship 1: timer simplification — replace 5-digit MM:SS clock display in vehicle deploy timer rows with a 10-step decile-chunk progress bar (red fill). clock/timer-instance.ts: drop digit + shadow widget builders + setReusableTimerColor + setReusableTimerSeconds; add buildReusableTimerBarBorder + buildReusableTimerBarFill + setReusableTimerProgress(cache, elapsedFraction). state/hud-cache-types.ts: ReusableTimerWidgetCacheEntry shape — drop 10 digit/shadow widget refs + lastDisplayedSeconds; add barBorder + barFill + lastDecile. vehicles/deploy-timer-ui.ts: two call sites compute elapsedFraction from total + remaining (ACTIVE_MAP_CONFIG.roundStartAirDelay or slot.respawnDelaySeconds); cache-shape visibility check updated; reset path uses lastDecile. READY/ACTIVE/SPAWNING/DEPLOYING modes unchanged (still toggle statusText + statusShadow via setReusableTimerStatus). Bar fills 0% -> 100% over cooldown duration; fires SetUIWidgetSize only on decile transition (~10 writes per countdown vs ~30-120 per-tick before). Net heap delta: -8 widgets per timer instance (was 10 digit/shadow widgets, now 2 bar widgets). Per pid: ~12 rows × -8 = -96 widgets/pid. Master clock untouched per L2.
// v1.438: Wave 4 v1.438 bugfix: showPlayerReadyPanelForPid was forcing all widgets visible=true AFTER refresh had set the CLAIM ADMIN widgets to invisible (per Admin.isAdminVacant() check in syncPlayerReadyPanelClaimAdminButtonForPid). When admin was in place, the button incorrectly appeared visible AND clickable on the panel because the show loop overrode the sync's correct hide. Fix: skip the 3 claim-admin widget IDs in the show loop -- their visibility is owned by the sync function. Now: admin in place = button hidden (any state). Admin vacant + pre-live = button visible + enabled. Admin vacant + live = button visible + greyed/disabled. Consistent across all states.
// v1.437: Wave 4 v1.3 refinement: restore one-time first-ever-join auto-admin exception per user clarification ('the first connection is an exception - I still want the host to be placed as admin the first time. I just want subsequent vacancies not filled.'). Admin slot auto-assignment now lives INSIDE the first-ever-join branch in onPlayerJoin (gated on _hostFirstPid === undefined so it fires exactly once per server lifetime). Once host is set, the auto-admin branch is permanently inert -- subsequent joiners landing on a vacant admin slot do NOT inherit it. Plan doc + state doc + design doc + MP tests all updated to reflect: Q2 REFINED (not superseded), L17 revised with explicit one-time-first-join branch, admin contract row corrected.
// v1.436: Wave 4 v1.3 follow-up: explicit-claim-only model + No Admin display. (1) Removed auto-admin-on-vacancy in Admin.onPlayerJoin -- supersedes Q2 (first-joiner-auto-admin). Server starts vacant; first joiner now sees the panel + must press CLAIM ADMIN like everyone else. No code path sets _currentAdminPid except claimAdmin(pid) called from a CLAIM ADMIN click. (2) Added pre-approved string twl.playerReadyPanel.noAdminLabel = 'No Admin'; vacant-slot Game Admin row now reads 'Game Admin: No Admin' instead of 'Unknown' (disambiguates intentional vacancy from disconnect-fallback). (3) GIVE UP ADMIN live-disable confirmed working in v1.435 (signature change picks up live:1 on startMatch re-render at conquest-flow.ts:50; click handler also has isMatchLive() defensive guard); v1.436 reaffirms. Plan doc updated to v1.3 (Q2 superseded; L17 revised; L19 added; locked dialog spec updated). Wave 4 ledger now spans v1.421-v1.436.
// v1.435: Wave 4 Ship 7: cleanup + docs + final polish. (1) GIVE UP ADMIN button live-disable: greyed + SetUIButtonEnabled(false) when match is live (mirrors RESET treatment in syncReadyDialogModeActionWidgetsForPid); click handler also guards on isMatchLive() defensively. Symmetry with the CLAIM ADMIN live-disable from v1.434. (2) Dropped unused twl.playerReadyPanel.claimButton string (orphaned by the v1.2 redesign that replaced universal Claim with explicit handoff). (3) Code sweep verified tryOpenReadyDialogForPlayer is the sole admin-gate chokepoint -- both interact-point and world-interactables green-smoke routes go through it; no bypass. (4) Doc updates: refreshed conquest_optimization_state.md (Project Stats + admin/identity.ts + player-ready-panel.ts file map rows + function inventory entries); refreshed conquest_design.md (admin contract architecture pattern row); appended Wave 4 entries to conquest_mp_ongoing_tests.md; finalized 5.01.26_conquest_wave_4_plan.md status. Wave 4 complete.
// v1.434: Wave 4 Ship 6 polish: CLAIM ADMIN button is now greyed + SetUIButtonEnabled(false) when match is live (mirrors READY button live-disable treatment in syncPlayerReadyPanelReadyButtonForPid). Click handler also gets a defensive isMatchLive() guard so even raw clicks slipping through cannot perform admin handoff during a live match (config is locked once live). Button stays visible when vacant + live so the viewer understands the slot is vacant -- just inert.
// v1.433: Wave 4 Ship 6 polish: panel READY button now auto-closes the panel after the toggle (mirrors CHANGE TEAMS pattern). Click READY -> panel hides + cursor restored + handleReadyDialogReadyButtonClick fires (world log + HUD refresh + auto-start). Drops the now-unnecessary post-toggle refresh since panel is hidden.
// v1.432: Wave 4 Ship 6 polish: force CLAIM ADMIN button to render on two rows via embedded newline (CLAIM\nADMIN), matching the visual style of CHANGE TEAMS / SPECTATE COACH which wrap naturally. Engine didn't auto-wrap CLAIM ADMIN at 140px because the text was just under the wrap threshold. User-requested visual polish.
// v1.431: Wave 4 Ship 6 (v1.2 redesign): explicit admin handoff replaces universal Claim. Admin slot is single-occupancy (drops _adminJoinList; new _currentAdminPid + isAdminVacant + claimAdmin + giveUpAdmin). New CLAIM ADMIN button on panel top-right (vacancy-gated visibility per L17/D4). New GIVE UP ADMIN button on existing ready dialog (left of RESET TO DEFAULT). New refreshAllVisiblePlayerReadyPanels broadcast helper called from claim, give-up, and admin-disconnect transitions. Reverted v1.425 SP test override (Admin.isAdmin no longer hard-returns false; real check restored). 2 new pre-approved strings (claimAdminButton, giveUpAdmin). Plan doc updated to v1.2 (L4/L9/L13/D2/R6 obsoleted; L17/L18/D4 added; Ship 6 redesigned).
// v1.430: Wave 4 Ship 5 polish: (1) READY button label now toggles READY <-> NOT READY based on State.players.readyByPid (mirrors syncReadyToggleButtonWidgetsForPid pattern from roster-render.ts). New syncPlayerReadyPanelReadyButtonForPid called from refreshPlayerReadyPanelContentForPid; also greys + disables the button when match is live. (2) CHANGE TEAMS now auto-closes the panel before swap (mirrors processReadyDialogSelection's hideReadyDialogUI:257) so the viewer isn't left looking at a stale uninteractable widget tree during the redeploy.
// v1.429: Wave 4 Ship 5: wire player-ready-up panel button click handlers via new sibling router (per L15). New file interaction/ui-events-player-ready-panel.ts with tryHandlePlayerReadyPanelButtonEvent dispatched from ui-events.ts after the full-dialog handler. CLOSE = hidePlayerReadyPanelForPid + setUIInputModeForPlayer(false). READY = handleReadyDialogReadyButtonClick (same handler as full dialog -- world log, HUD count, auto-start gate fire identically) + refresh panel status row. CHANGE TEAMS = swapPlayerTeam (force not-ready, processReadyDialogSelection redeploy) + refresh panel status row. SPECTATE/COACH = no-op claim (defensive; SetUIButtonEnabled(false) should suppress engine-side already). Independent debounce tracker (playerReadyPanelLastPrimaryClickByPid) so panel + dialog clicks don't cross-cancel.
// v1.428: Wave 4 Ship 4: wire player-ready-up panel dynamic content (Game Host, Game Admin, ready status). New refreshPlayerReadyPanelContentForPid(pid) reads live state and updates the 3 dynamic labels via safeSetUITextLabel; called from showPlayerReadyPanelForPid before the visibility flip so no stale flash. Host/Admin: pass live Player as format arg if pid is connected, else fall back to unknownPlayer stringkey ('Unknown'). Status: yes/no format keyed off State.players.readyByPid[pid] with team name substituted via getTeamNameKey. SetUITextAnchor re-applied per-update to mirror roster pattern. Also: button row position fix BUTTON_ROW_Y 52 -> 8 (BottomCenter convention is widget-bottom-aligns-parent-bottom + positive-Y-up; v1.427 over-shot and overlapped status row). New gap status-bottom -> button-top = 44px, 8px clearance below panel.
// v1.427: Wave 4 Ship 3 polish v2: fix button row position + size. (1) Direction: BUTTON_ROW_Y is positive-Y-up under BottomCenter anchor (widget center at parent's bottom edge); v1.426 went the wrong way. New BUTTON_ROW_Y=+52 places the 72-tall button row 16px above panel bottom. (2) Doubled button height 36 -> 72 so wrapped text ('SPECTATE / COACH') fits comfortably; width bumped 130 -> 140 (4 x 140 + 3 x 12 = 596px row, fits in 640px panel with 22px side margin). Cannot truly double width without breaking the 4-button row layout.
// v1.426: Wave 4 Ship 3 polish: fix player-ready-up panel layout bugs exposed by SP test (v1.425). (1) Strings: replace nested msg(format, msg(...)) calls with bare-stringkey format args -- nested Messages return undefined and trip ui-helpers normalizer fallback to readyText (showed 'YOU ARE READY' on all 3 rows). (2) Centering: re-apply SetUITextAnchor post-build (matches roster pattern) since SetUITextSize resets the anchor; switch host/admin/status to widget-anchor Center + posX=0 + textAnchor=Center. (3) Buttons: raise BUTTON_ROW_Y from -16 to -30 so the 36-tall button row clears the panel bottom (was clipped 2px below).
// v1.425: TEMP SP TEST: force Admin.isAdmin to return false so the SP host pid hits the non-admin route in tryOpenReadyDialogForPlayer and the new player-ready-up panel renders for visual verification. REVERT before further Wave 4 work or MP playtest.
// v1.424: Wave 4 Ship 3: triple-tap router branches on Admin.isAdmin -- non-admins now route to the small player-ready-up panel; admins continue to the full ready dialog. First user-visible Wave 4 behavior change.
// v1.423: Wave 4 Ship 2: Player Ready Up Panel module skeleton + lazy-build registry integration. New file src/ready-dialog/player-ready-panel.ts (~17 widgets per pid). 6 new strings (twl.playerReadyPanel.*) added per L11. 16 widget-ID constants (UI_PLAYER_READY_PANEL_*) added to strings/ui-ids.ts. lazy-build-registry.ts gets 5 schema additions + new case. cleanupHudForPid hook added for disconnect cleanup. Triple-tap routing unchanged in this ship; Ship 3 wires admin-gate.
// v1.422: Ship 1 cleanup + AGENTS.md update: removed dead [ship1-diag] console.log lines (world log + console.log unreliable per new AGENTS.md Debugging/Diagnostic Output Policy section); removed Admin.snapshotForDiag (no consumer).
// v1.421: Wave 4 Ship 1: admin/host identity foundation. New Admin namespace in src/admin/identity.ts with module-level state (_adminJoinList, _hostFirstPid, _hostNameMessage). Public API: onPlayerJoin, onPlayerLeave, isAdmin, getCurrentAdminPid, isHost, getHostFirstPid, getHostNameMessage, claimAdminForPid, snapshotForDiag. Wired into onPlayerJoinGameImpl + onPlayerLeaveGameImpl. No UX surface yet; pure plumbing. Temporary [ship1-diag] console.log lines on join/leave for SP smoke verification (removed in later ship).
// v1.420: Wave 3 documentation consistency sweep: stale entries fixed in conquest_optimization_state.md (warmPrimeActiveByPid lifecycle row, engagedObjIdByPid deleter list, confirmReadyDialogModeConfig comment), conquest_optimization_analysis.md M7 (now 11 records, was 12), conquest_issues_summary.md (#32 + #33 + #76-partA resolved-by-removal; #105 + #40 fix mechanism replaced + caveat; #109 sync), conquest_issues.md (#76 + #105 + #40 long-form Wave 3 caveats).
// v1.419: Wave 3 wrap-up: documentation sweep + warmHiddenReadyDialogCacheForPid deletion (dead since v1.417 follow-up). State doc function inventories updated for actions.ts, hud-warm-state.ts, player-deploy.ts, player-loop-inputs.ts, loading-overlay.ts (stub), state/core.ts, dialog-build.ts, lifecycle.ts. conquest_design.md Loading Gate / Warm Token entries replaced with Lazy-Build Dispatch pattern. conquest_optimization_analysis.md M2 + D2 + architecture references updated. Wave 3 plan doc marked COMPLETE. #109 status updated to await-MP-confirmation.
// v1.418: Ship 8: loading-gate deletion (combined Ship 8a dead-helper sweep + Ship 8b gate machinery removal). Players snap directly to deploy screen on first-join and team-swap; no loading overlay UX. Gate orchestration, hud-warm-state accessors, prebuildAllUiFamiliesHidden, runLoadingGateUntilReady, releaseLoadingGate, team-swap waits, mod.EnableAllInputRestrictions wrapper, primeReadyDialogRevealWhileBlocked, and loading-overlay.ts all deleted. isHudWarmReadyForPid + isHudSwapTransitionActiveForPid kept as constant readers (true/false) for backward-compat across clock/help/vehicles/HUD pipeline.
...
// v1.273: fix: sink vehicles to y=-1000 before DealDamage at startup cleanup and prior-vehicle destroy sites (avoids audible explosions at pad positions)
// v1.272: fix: replace vehicle UnspawnObject with DealDamage at startup cleanup and prior-vehicle destruction (UnspawnObject on transitional vehicles emits engine-side errors that try/catch cannot suppress)
// v1.271: fix: relocateSlotSpawner waits 2s for engine init before configure (fixes transport-3 Abrams-instead-of-selected-vehicle bug when heli/ground toggle forces pad relocation)
// v1.270: fix: preserve X/Z when teleporting vehicles down at countdown (avoid minimap sweep artifact)
// v1.269: Revert v1.268: Abrams-reject guard caused spawn-count regression (non-tank slots producing default Abrams due to stuck spawner configs were rejected into empty slots rather than a visible wrong vehicle). Also revert X/Z-preserving teleport. Back to v1.265 spawner behavior — wrong-vehicle visible at countdown start, but no slots are empty. Fresh diagnostic pass needed before re-attempting a fix.
// v1.268: Countdown-start reset now teleports vehicles straight down (preserve X/Z, Y=-1000) instead of to map-center to avoid minimap slide artifact. Re-add Abrams-reject guard in bindSpawnedVehicleToExpectingSlot, but sink-then-delayed-damage (teleport to y=-1000 preserving X/Z, then Timers.setTimeout(DealDamage, 500ms)) so rejected default Abrams don't produce audible explosions at the pad. relocateSlotSpawner and Phase C re-config are NOT re-introduced (caused v1.266 spawn regressions).
// v1.267: Revert v1.266 hot-fix attempt: SetObjectTransform relocate + Phase C re-config + Abrams-reject guard caused new regressions (5 of 8 slots failed to spawn at map start, rejected-Abrams DealDamage produced audible explosions on retries). Back to v1.265 spawner behavior while we rethink the transport-slot Abrams substitution.
// v1.266: Fix Abrams-substitution regression on transport slots: relocateSlotSpawner now uses SetObjectTransform in-place (restores v1.230 fix) instead of UnspawnObject+SpawnObject, eliminating the AutoSpawn default-Abrams race during the engine's init window on relocated spawners. Belt-and-suspenders: resetVehicleSlotsAtCountdownStart Phase C re-pushes configureVehicleSpawner before each dispatch. Last-line defense: bindSpawnedVehicleToExpectingSlot rejects and damages stray default Abrams when the expecting slot's intended type is not a tank (CQ_Bug_49 guard restored).
// v1.265: Move vehicle reset from LIVE-start to countdown-start: sink to y=-1000, wait 0.5s for teleport to land, then DealDamage. Fresh spawns complete during countdown (no jumble at LIVE!). Removed dead destroyAllTrackedVehicles helper.
// v1.264: Move vehicle reset from LIVE-start to countdown-start: sink to y=-1000, wait 0.5s for teleport to land, THEN DealDamage — no explosion sounds at pads, fresh spawns complete during countdown (no jumble at LIVE!). Removed dead destroyAllTrackedVehicles helper.
// v1.263: Move vehicle reset from LIVE-start to countdown-start: sink vehicles to y=-1000, wait 0.5s for teleport to land, THEN DealDamage — no more explosion sounds at pad positions. Fresh spawns complete during the countdown so there's no jumble at LIVE!. Removed dead destroyAllTrackedVehicles helper.
// v1.262: Live-start fleet reset: sink pre-live vehicles to y=-1000 then DealDamage (avoids UnspawnObject; wreckage below map), clear respawn clocks, enqueue fresh dispatches with no cooldown so live-round fleet is ready by LIVE-banner fade. Passive vehicle HUD auto-reveal: deployed viewers now surface the timer HUD whenever a slot is cooling down — prior gating required currentlyVisible to stay visible, so a hidden HUD at deploy time could never resurface. Add AH6M_Pax, DirtBike, DirtBike_Pax vehicle types across classification, deploy-timer labels, ready-dialog knob options, and strings. Firestorm presets: replace Quadbike → DirtBike (West) / DirtBike_Pax (East) and swap Team2 AH6M → AH6M_Pax across all matchup sizes.
// v1.261: Fix v1.259 vanilla spawner: global setTimeout does not exist in Portal sandbox, so the timeout promise threw and rejected the first doDispatch — F16 bound but orientation teleport never ran, and the mutex chain's .then() propagated rejection so no subsequent slot dispatched. Switch to Timers.setTimeout, wrap Promise.race in try/catch, and route every mutex enqueue through enqueueDispatch() which appends .catch(() => {}) so a single failed dispatch cannot poison the chain.
// v1.259: Vanilla vehicle spawner rewrite: serial dispatch via Promise mutex, event-driven bind, Clocks-based respawn. Deletes deploy-fulfillment, reservations, 20-retry loop, 5s poll, 3-path bind cascade. Non-Vanilla deploy paths removed.
// v1.258: Vanilla vehicle spawner rewrite: serial dispatch via Promise mutex, event-driven bind, Clocks-based respawn. Deletes deploy-fulfillment, reservations, 20-retry loop, 5s poll, 3-path bind cascade. Non-Vanilla deploy paths removed (see reference_conquest_attempt_b for history).
...
// v1.249: diag: BH test spawns 10m in front of player instead of static coords, increased settle times to 1s each before teleport+seat
// v1.248: diag: BH test buttons now teleport player to vehicle + Wait(0.5s) settle + ForcePlayerToSeat — full spawn+teleport+seat flow
// v1.247: diag: extend BH test buttons with ForcePlayerToSeat — proves full spawn+seat flow in isolation
// v1.246: diag: add 4 isolated BountyHunter spawn test buttons to admin panel (F16/AH-6M/DirtBike/BlackHawk) — zero contact with existing vehicle infrastructure, proves whether BountyHunter pattern works independently
// v1.245: fix: rewrite spawnFreshAircraftDirectSpawnVehicleForSlot with BountyHunter pattern — 2s delay, cleanup default Abrams, AutoSpawn(true) with 3-call-only config (no configureVehicleSpawner, no ForceVehicleSpawnerSpawn). v1.244 missed this path because HQ Deploy of aircraft routes through the fresh-spawner path, not forceSpawnWithRetry
// v1.244: fix: apply BountyHunter spawner initialization pattern — 2s delay between SpawnObject and configuration at startup + AutoSpawn fallback in forceSpawnWithRetry for vehicle types where ForceVehicleSpawnerSpawn silently fails (F22/MH6/Eurocopter/JAS39)
// v1.243: diag: PREP row now differentiates silent-bail branches inside tryFulfillPendingVehicleDirectSpawnSeatForPlayer (fulfill-gotveh/seatocc/notready/nospawn/token/ok) so we can tell WHY F22/MH6/MH6_Pax HQ Deploy stalls after fulfill-entry with all other rows blank, while F16 progresses to SPAWN=ok/BIND=ok/SEAT=verify-ok. Live-terminal alive path bypasses preparePendingVehicleDirectSpawnVehicleForPlayer, so all fulfill-path bails were previously invisible
// v1.242: diag: PREP row confirmatory markers (entered/fulfill-entry/fulfill-no-slot/fulfill-bad-mode/fulfill-cant) to trace which deploy path aircraft HQ Deploy takes — v1.241 showed PREP blank for all three aircraft (F16/F22/MH6) including the working F16, suggesting the click path bypasses preparePendingVehicleDirectSpawnVehicleForPlayer and routes through OnPlayerDeployed→tryFulfillPendingVehicleDirectSpawnSeatForPlayer instead
// v1.241: diag: Phase 1 Part A follow-up — PREP row now distinguishes hud-blocked vs no-slot vs specific isVehicleSlotReadyForReservationDeploy failure reason (enabled/vid/expecting/respawn/retry/timer/gated) so F22 PREP=failed and MH6 PREP=blank can be disambiguated. No production path change; diagnostic-only, strips with FEATURE_DEPLOY_DIAGNOSTIC.
// v1.240: diag: Phase 1 Part A — per-player deploy-flow trace HUD wired into production deploy path (Vehicle/Mode/Click/Prep/Spawn/Bind/Deploy/Seat rows) so working F16 HQ Deploy can be compared side-by-side with failing F22/AH64/MH6. FEATURE_DEPLOY_DIAGNOSTIC=true (enabled for testing), FEATURE_MIN_SPAWN_TEST=false (Part B 6-variant matrix exhausted, all failed, bundle budget reclaimed). Call sites guarded by `if (FEATURE_DEPLOY_DIAGNOSTIC)` so flag=false ships production-identical to v1.238.
// v1.238: diag: rewire test buttons as variant matrix (BASE/S-1/TP0/TP5/TPW) on F16; post-seat verification via world-log
// v1.236: diag: admin-panel isolated minimal-pattern test button (5 vehicles: F16/AH64/F22/MH6/Abrams) runs canonical SDK spawn+teleport+seat WITHOUT workaround layers (no activeSpawn, no CQ_Bug_49 reject, no yaw correction) to test if SDK 1.2.3 patches made our workaround layer harmful. FEATURE_MIN_SPAWN_TEST + FEATURE_DEPLOY_DIAGNOSTIC default OFF; production path identical to v1.234.
// v1.234: Fix: DirtBike/DirtBike_Pax/AH6M_Pax missing from resolveVehicleSlotSpawnCategory caused deploy menu filter + bootstrap auto-spawn bugs
// v1.233: Fix v1.230 regression: relocateSlotSpawner now re-applies VEHICLE_SPAWN_YAW_OFFSET_DEG so transport-slot heli/fastmover flips produce correctly-oriented vehicles (AH6M oriented-wrong symptom). Also fix v1.229 gaps: add DirtBike/DirtBike_Pax/AH6M_Pax cases to getVehicleDeployTimerLabelKey (previously fell through to 'unknown'), add AH6M_Pax to isAircraftVehicleType + doesVehicleTypeSupportAirDeploy + isAircraftVehicleInstance so heli-slot logic classifies it correctly.
// v1.232: Add DirtBike + DirtBike_Pax to isFastMoverDirectSpawnVehicle so HQ-deploy seat-retry falls back to seat -1 on two-wheel vehicles where driver seat index may not be 0. Addresses the DirtBike seat-failure + orphan-vehicle symptom reported in v1.230 testing.
// v1.231: Cleanup orphan vehicles at live transition: sweep any mod.AllVehicles entry not tracked by State.vehicles.vehicleToSlot at startMatch. Fixes pre-match HQ-deployed vehicles (Jeep, DirtBike) persisting into live when seat-bind failed and the vehicle was never mapped to a slot.
// v1.230: Phase A: relocateSlotSpawner now uses mod.SetObjectTransform in place instead of UnspawnObject+SpawnObject, eliminating the AutoSpawn Abrams race on every ready-dialog vehicle-type change. Architecture validation gate for the relocate-and-reuse plan.
// v1.229: Add AH-6M PAX to chopper options; add Dirt Bike and Dirt Bike PAX to fast-mover options
// v1.228: Revert vehicle spawn/bind stack to pre-v1.223 baseline (commit 3aa9edf). Iterative v1.223-v1.227 fixes did not resolve Abrams-substitution + seat-fail regressions on Air/Forward Deploy. Known MP singleton race tracked for future re-architecture per design_doc/vehicle_spawn_bind_rearchitecture.md.
// v1.228: Revert vehicle spawn/bind stack (spawner-bind, spawner-sequence, deploy-fulfillment, vehicle-events, runtime-state, runtime-types, spawner-slots) to pre-Phase-1 baseline (commit 3aa9edf). Iterative fixes in v1.223-v1.227 did not resolve Abrams-substitution + seat-fail regressions on Air/Forward Deploy. Known MP singleton race returns as a tracked issue; re-architecture notes saved in design_doc/vehicle_spawn_bind_rearchitecture.md for a future clean restructure.
// v1.227: fulfillment poll 1s→2s + fresh-air radius 40m→250m to catch late jet binds
// v1.226: class-aware bind fallback (3rd-tier: match by vehicle class when multiple slots expecting) + Abrams-specific intercept (CompareVehicleName, not tank-class) — fixes v1.224 regression where aircraft orphaned and tank-type slots (Leopard/CV90/Bradley) still Abrams-substituted
// v1.224: v1.224 hotfix — fix Phase 1 Air/Forward regression: single-expecting fallback for velocity-displaced aircraft, extend Abrams intercept to non-tank slots
// v1.223: CQ_Bug_ActiveSpawnSingletonMPRace: replace global activeSpawn* singleton with per-slot expectingSpawn+lastRequestedSpawnPos lookup so concurrent MP Air/Forward clicks bind to their own slots
