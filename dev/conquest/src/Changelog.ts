...
// v0.856: Phase 7 live deploy follow-up: move the modal chrome to a dedicated right-side backplate with the close action below the row stack, document the pending HQ visibility constraint for shared authored terminals, and harden direct vehicle spawns by clearing stale unbound vehicles off a slot pad before forcing a fresh spawn
// v0.855: Phase 7 live deploy terminals: enable odd main-base world terminals, reuse the deploy HUD as an in-world alive-player menu with a modal backplate and close button, and route slot claims through direct seat fulfillment without undeploying the player
// v0.854: Phase 7 world terminal cleanup: keep even main-base terminals interactable, explicitly disable odd and point authored interact points until their menus are implemented, and sync the world-interactable docs to the current shipped state
// v0.853: Phase 7 world terminal labels: reuse the shared DEPLOY UI string for odd main-base world icons instead of a one-off VEHICLE label while keeping READY on even ids
// v0.852: Phase 7 world terminal labels: set authored world-icon text for main-base interactables so even ids show READY and odd ids show VEHICLE while keeping only the ready terminals interactable for now
// v0.851: Phase 7 ready terminals: switch even main-base ready-dialog terminals to authored WorldIcon plus authored InteractPoint pairs keyed by the same numeric id, remove the runtime-spawned interact-point path, and have script simply enable and route the authored interact points into the existing ready-dialog flow
// v0.850: Phase 7 ready terminals tuning: validate spawned runtime interact-point refs before suppressing reseed attempts and place the even main-base ready-dialog interact points directly on the authored world-icon anchor with explicit unit scale to maximize the chance that the engine exposes the interact prompt
// v0.849: Phase 7 ready terminals hardening: keep retrying runtime interact-point seeding for authored even main-base world icons during startup and once-per-second gameplay ticks so ready-dialog terminals still become interactable if authored world icons are not queryable on the first startup pass
// v0.848: Phase 7 ready terminals: keep even main-base world icons authored in Godot, spawn runtime interact points from the authored icon world positions, and route those spawned interact points back into the existing ready-dialog path without overriding authored icon art
// v0.846: Phase 7 ready terminals fix: import the world-interactables module into the Conquest entrypoint so even main-base terminal routing and ready-marker icon presentation are included in the emitted bundle
// v0.845: Phase 7 ready terminals: route even main-base world-interactable ids through the existing ready-dialog open path and derive a green ready-marker world icon from the map-config parity rubric while leaving authored team visibility ownership in Godot
// v0.844: Phase 7 Firestorm interactable authoring: populate the authored main-base interactable ObjId array with 1000-1015 and the flag interactable ObjId array with 1050-1055 so Firestorm now carries the seeded Phase 7 world-interactable ids in map config
// v0.843: Phase 7 world-interactable schema pass: add explicit main-base and flag interactable ObjId arrays to MapConfig, derive menu actions from the locked range and even-odd rubric, add validation for duplicate and out-of-range interactable ids, and seed Firestorm placeholders for authored Phase 7 interactables while aligning the design docs to the new first-pass array-based contract
