...
function updateHelpTextVisibilityForPid(pid: number): void {
    ...
    const helpText3 = safeFind(wn("HelpText3", pid));
    if (helpText3) {
        safeSetUIWidgetVisible(helpText3, showHelp);
        safeSetUITextLabel(helpText3, msg(mod.stringkeys.twl.hud.helpText3));
    }

    const helpText = safeFind(wn("HelpText", pid));
    if (helpText) {
        safeSetUIWidgetVisible(helpText, showHelp);
        safeSetUITextLabel(helpText, msg(mod.stringkeys.twl.hud.helpText));
    }

    const helpText2 = safeFind(wn("HelpText2", pid));
    if (helpText2) {
        safeSetUIWidgetVisible(helpText2, showHelp);
        safeSetUITextLabel(helpText2, msg(mod.stringkeys.twl.hud.helpText2));
    }
