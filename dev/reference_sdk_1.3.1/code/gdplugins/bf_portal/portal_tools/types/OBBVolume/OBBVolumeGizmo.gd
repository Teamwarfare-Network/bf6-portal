...
func _redraw(gizmo: EditorNode3DGizmo) -> void:
	...
	var point_a = half_extents * Vector3(-1, -1, 1)
	var point_b = half_extents * Vector3(-1, 1, 1)
	var point_c = half_extents * Vector3(1, -1, 1)
	var point_d = half_extents * Vector3(1, 1, 1)
	var point_e = half_extents * Vector3(1, -1, -1)
	var point_f = half_extents * Vector3(1, 1, -1)
	var point_g = half_extents * Vector3(-1, -1, -1)
	var point_h = half_extents * Vector3(-1, 1, -1)

	var faces = [
		[point_d, point_f, point_e, point_c], # right
		[point_b, point_d, point_f, point_h], # top
		[point_b, point_d, point_c, point_a], # front
		[point_h, point_b, point_a, point_g], # left
		[point_a, point_c, point_e, point_g], # bottom
		[point_f, point_h, point_g, point_e], # back
	]
