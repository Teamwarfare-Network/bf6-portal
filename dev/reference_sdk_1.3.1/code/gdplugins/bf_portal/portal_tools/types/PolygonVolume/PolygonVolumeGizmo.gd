...
func _get_closest_segment(screen_position: Vector2, volume: PolygonVolume, camera: Camera3D) -> ClosestSegmentResult:
	...
	if camera.projection == Camera3D.ProjectionType.PROJECTION_PERSPECTIVE:
		...
		# problem reduces to 2D without worrying about height offset
		var cursor_world = camera.project_position(screen_position, 1.0)
		var cursor_2d = Vector2(cursor_world.x, cursor_world.z)
		for i in range(len(points)):
			var j = (i + 1) % len(points)
			var curr_point = points[i]
			var next_point = points[j]
			var curr_point_world = volume.to_global(_get_planar_point(curr_point))
			var next_point_world = volume.to_global(_get_planar_point(next_point))
			var curr_point_2d = Vector2(curr_point_world.x, curr_point_world.z)
			var next_point_2d = Vector2(next_point_world.x, next_point_world.z)

			var result: Vector2 = Geometry2D.get_closest_point_to_segment(cursor_2d, curr_point_2d, next_point_2d)
			var distance_from_cursor_to_segment = (result - cursor_2d).length_squared()

			if distance_from_cursor_to_segment < min_distance and distance_from_cursor_to_segment <= acceptable_distance:
				min_distance = distance_from_cursor_to_segment
				min_segment.x = i
				min_segment.y = j
				target_point = Vector3(result.x, 0, result.y)
