...
@onready var experience_exporter_export: Button = %CreateCustomMode_Button
@onready var export_level_label: Label = %ExportLevel_Label
@onready var experience_exporter_export_label: Label = %CreateCustomMode_Label

var _window_experience_exporter_export = preload("res://addons/bf_portal/experience_exporter/experience_exporter_export.tscn")
var _export_window: Window
...
func _ready() -> void:
	...
	if not experience_exporter_export.pressed.is_connected(_on_experience_exporter_export):
		experience_exporter_export.pressed.connect(_on_experience_exporter_export)
...
func _on_experience_exporter_export():
	
	if not _export_window:
		_export_window = _window_experience_exporter_export.instantiate()
		add_child(_export_window)
		_export_window.portal_dock = self
		_export_window.popup()
			
func set_exporter_label(mod_count: int):
	if mod_count == 0:
		experience_exporter_export.disabled = true
	else:
		experience_exporter_export.disabled = false
	experience_exporter_export_label.text = "%s available" % mod_count
