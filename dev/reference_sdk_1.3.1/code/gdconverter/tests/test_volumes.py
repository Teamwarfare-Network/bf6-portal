...
from tests.utils import TEST_GD_LEVELS, check_object_equals, check_polygon_equals, setup_mock_project
...
def test_polygon_volume(tmp_path: Path) -> None:
    ...
    polygon_rotated: dict[str, Any] = exported_data["Portal_Dynamic"][1]
    ...
    check_polygon_equals(expected, polygon)

    expected_rotated: dict[str, Any] = {
        "type": test_type,
        "name": f"{test_type}Rotated",
        "id": f"Parent/{test_type}Rotated",
        "height": 0,
        "points": [
            {"x": 1, "y": 0, "z": -1},
            {"x": -3, "y": 0, "z": -5},
    ...
    check_polygon_equals(expected_rotated, polygon_rotated)
    ...
    polygon_rotated = exported_data["Portal_Dynamic"][1]
    polygon["id"] = test_type
    polygon_rotated["id"] = f"Parent/{test_type}Rotated"
