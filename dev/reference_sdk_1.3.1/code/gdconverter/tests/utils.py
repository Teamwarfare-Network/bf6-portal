...
def check_object_equals(expected: dict[str, Any], obj: dict[str, Any], skip_props: list[str] | None = None) -> None:
    ...
    for prop, data in expected.items():
        if prop in skip_props:
            continue
        assert prop in obj
        assert data == obj[prop], f"Mismatch: \n\tProperty: {prop}\n\tExpected: {data}\n\tFound:    {obj[prop]}"
...
def check_polygon_equals(expected: dict[str, Any], obj: dict[str, Any]) -> None:
    epsilon = 0.000001
    assert "points" in obj
    assert "points" in expected
    components = ["x", "y", "z"]
    for point_a, point_b in zip(expected["points"], obj["points"], strict=True):
        for component in components:
            assert point_a[component] - point_b[component] < epsilon
    check_object_equals(expected, obj, ["points"])
