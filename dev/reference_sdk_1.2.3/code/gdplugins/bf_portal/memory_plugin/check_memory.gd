...
@onready var table: MemoryTable = %DataTable
@onready var toggles = %ColumnToggles
@onready var _summary_box = %Summary
...
func set_current_scene(scene: Node):
	...
	var total_types = types_and_names.size()
	var total_memory = 0
	...
	_summary_box.text = (
		"""Total Objects: %d
Total Object Types: %d
Total Physics Cost: %d"""
		% [total_instances, total_types, total_memory]
	)
